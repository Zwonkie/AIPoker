...
- .to_tensors
- .forward
- check_no_nan_or_crash
- HandRecordV4
- compute_range_aware_equity
- simulator.py
- _get_preflop_ranked
- _realization_discount
- PokerAction
- .to_tensors
- .to_tensors
- .to_tensors
...
- `RECT` --uses--> `PokerVision`  [INFERRED]
  PHPHelp.py → core/vision.py
- `RECT` --uses--> `XMLTracker`  [INFERRED]
  PHPHelp.py → core/xml_tracker.py
...
Cohesion: 0.20
Nodes (8): V13 — MILESTONE (kept reference / fallback), Observed-GOOD — KEEP, do not regress in V14, P2 — Preflop flattening (partly resolved by P1, finish the rest), P3 — AGG axis (learned aggression), Root cause this version exists to fix, Tooling to add alongside V14, V14 — Specs & Observations, Validation plan (how we'll know P1 worked)
...
Cohesion: 0.08
Nodes (13): build_opponent_pool(), _call_amount_from_pot_odds(), describe_pool_entry(), HeuristicOpponent, NNOpponent, Opponent, Uniform Opponent interface for SixMaxSimulator's 5 opponent seats (V18).  Replac, A pure scripted archetype bot (fish/maniac/nit/tag heuristics). (+5 more)
...
Cohesion: 0.12
Nodes (17): V21 manifest — clone of V20_preflopEq_AI (versions/v20_preflopEq_AI). IDENTICAL, PokerEVModelV4, Args:             hole: [batch, 2] (integer card indices)             board: [ba, Decision Transformer model for Pluribus V4.     Processes sequences of (State, A, Generate a standard causal mask (future states are True, which means masked out), main(), Pure-policy (deployed) winrate eval for V12-D.  Training BB/100 is dragged by ex, run_field() (+9 more)
...
Cohesion: 0.20
Nodes (16): ContractV12, Implements the 37-feature context extraction for Pluribus V12 models (V20_preflo, argmax_label(), entropy(), fmt_dist(), kl(), load_model(), main() (+8 more)
...
Cohesion: 0.19
Nodes (8): V16 manifest — clones V15 (versions/v15).  V16 keeps V15's discretized 6-action, PokerEVModelV4, Args:             hole: [batch, 2] (integer card indices)             board: [ba, Decision Transformer model for Pluribus V4.     Processes sequences of (State, A, Generate a standard causal mask (future states are True, which means masked out), main(), Pure-policy (deployed) winrate eval for V12-D.  Training BB/100 is dragged by ex, run_field()
...
Cohesion: 0.21
Nodes (13): cell_target(), deal_opponent(), hand_to_cards(), main(), [V43 / B2] Which risk-dampener actually controls the heads-up push/fold commit t, Per-action penalty vector for the non-fold actions, in bb. `committed_bb[i]` is, The actor target training would build from these counterfactual EVs, reproducing, Average target distribution over `samples` opponent hands at one (hand, stack) c (+5 more)
...
Cohesion: 0.12
Nodes (18): Live inference engine for V40 (versions/v40) -- the [BET-3] package from the 202, V40 manifest -- the [BET-3] package from the 2026-07-20 Fable full-stack review, PokerEVModelV4, Args:             hole: [batch, 2] (integer card indices)             board: [ba, Decision Transformer model for Pluribus V4.     Processes sequences of (State, A, Generate a standard causal mask (future states are True, which means masked out), main(), Pure-policy (deployed) winrate eval for V12-D.  Training BB/100 is dragged by ex (+10 more)
...
Cohesion: 0.15
Nodes (12): 1. Directory Structure, 2.5 Bot models (NN) and version management, 2. When to Create or Update Memory, 3. Formatting Memory Files (OKF Standard), 4. Memory Directory Index, Architecture & process, Current model line (V11 bridge → **V41 live**), Fable full-stack review (2026-07-20, V29 audit — cross-references the backlog, read alongside it) (+4 more)
...
Cohesion: 0.25
Nodes (7): 1. Simulation & Training  (`versions/v15/self_play/`), 2. Live Play  (`core/decision.py`, `core/action_executor.py`, `PHPHelp.py`), Context, Guidelines, Pipeline Flow — Simulation/Training vs Live Play, Train ≡ Serve invariants (must stay paired across both diagrams), [V42_liveFixes, 2026-07-21] Live-path corrections — what changed in LB2/LB3/LP1/LD3
...
Cohesion: 0.33
Nodes (3): Evaluates a 5, 6, or 7 card hand. Returns score and rank description.         Lo, Calculates hand equity using Monte Carlo simulations.         If specific_oppone, Converts standard 2-char string (e.g. 'Ah', 'Kd') to treys Card int.         Han

### Community 460 - ".predict_ev"
Cohesion: 0.29
Nodes (4): PokerEVModelV4, Args:             hole: [batch, 2] (integer card indices)             board: [ba, Decision Transformer model for Pluribus V4.     Processes sequences of (State, A, Generate a standard causal mask (future states are True, which means masked out)

### Community 461 - ".predict_ev"
Cohesion: 0.36
Nodes (7): decide_postflop_original(), decide_postflop_patched(), main(), measure(), [BET-1] calibration probe: does making the opponent bots' "value raise regardles, Exact copy of FuzzyPlayerArchetype.decide_postflop's facing-a-bet branch, with O, Exact copy of the pre-fix (V22 and earlier) facing-a-bet branch, for baseline co

### Community 462 - ".predict_ev"
Cohesion: 0.36
Nodes (7): decide_postflop_original(), decide_postflop_patched(), main(), measure(), [BET-1] calibration probe: does making the opponent bots' "value raise regardles, Exact copy of FuzzyPlayerArchetype.decide_postflop's facing-a-bet branch, with O, Exact copy of the pre-fix (V22 and earlier) facing-a-bet branch, for baseline co
...
Cohesion: 0.29
Nodes (6): Context, Fable Review — Live Serving Path (V29 "Herocules"), HIGH, LOW, MED, What's actually solid

### Community 465 - "Fable Review — Simulation Layer (V29)"
Cohesion: 0.29
Nodes (6): Context, Fable Review — Simulation Layer (V29), HIGH severity, LOW severity, MED severity, What's actually solid

### Community 466 - "Fable Review — Training, Targets & Evaluation (V29)"
Cohesion: 0.29
Nodes (6): Context, Fable Review — Training, Targets & Evaluation (V29), HIGH, LOW, MED, What's actually solid

### Community 467 - "simulator.py"
Cohesion: 0.29
Nodes (7): 1. Primary goal — opponent adaptation (fix the one known limitation), 1a. Range-aware equity (recommended, principled), 1b. Fallback if 1a underperforms, 2. Secondary improvements (each behind its own test), 3. Required validation tests (gate every change), 4. Open research questions (not blocking, worth investigating), V13 — Specification & Validation Plan

### Community 468 - "simulator.py"
Cohesion: 0.29
...
Cohesion: 0.33
Nodes (3): Known limitations carried forward — do NOT read this tag as "solved", V41 — MILESTONE (kept reference / fallback), Why it earns the tag
...
Cohesion: 0.33
Nodes (5): Change 1 [review #1, BET-3]: a single check no longer ends the betting round, Change 2 [review #2, BET-3]: CALL is no longer exempt from the risk penalty or the continuation credit, Change 3 [review #3, STACK-3]: ALLIN critic-consistency veto rescoped, Status, V40 SPECS — the [BET-3] package from the Fable V29 review

### Community 478 - "Fable Review — Data Contract / Feature Encoding (V29)"
Cohesion: 0.40
...
Cohesion: 0.40
Nodes (5): P1 — THE ATOMIC FLIP — TRAINING PIPELINE DONE + VERIFIED (2026-07-14), P1 (THE SPINE) — Discretized bet-size action space, P1a (REQUIRED sub-part) — Per-size counterfactual EV targets, P1b (REQUIRED sub-part / HARD prerequisite) — Size-SENSITIVE opponents, P1c — Live sizing consistency (train/serve — the recurring trap)

### Community 482 - "compute_range_aware_equity"
Cohesion: 0.36
Nodes (7): decide_postflop_original(), decide_postflop_patched(), main(), measure(), [BET-1] calibration probe: does making the opponent bots' "value raise regardles, Exact copy of FuzzyPlayerArchetype.decide_postflop's facing-a-bet branch, with O, Exact copy of the pre-fix (V22 and earlier) facing-a-bet branch, for baseline co
...
Cohesion: 0.83
Nodes (3): clean_card(), main(), parse_phhs_file()

### Community 510 - ".to_tensors"
Cohesion: 0.36
Nodes (7): decide_postflop_original(), decide_postflop_patched(), main(), measure(), [BET-1] calibration probe: does making the opponent bots' "value raise regardles, Exact copy of FuzzyPlayerArchetype.decide_postflop's facing-a-bet branch, with O, Exact copy of the pre-fix (V22 and earlier) facing-a-bet branch, for baseline co
