"""WS/JSON API for the native service. Localhost only.
...
Methods (v1):
  ...
  capture       {roi?: [x,y,w,h]}          -> {png_b64, size}       # occlusion-proof frame
  click         {x, y}                     -> {screen}              # client-relative
...
from live2.service import capture, interact
...
class Service:
    ...
    def click(self, x, y):
        return interact.click(self._bound(), float(x), float(y))
...
async def main():
    ...
    async with websockets.serve(lambda ws: _handle(ws, svc), HOST, PORT):
        print(f"live2.service listening on ws://{HOST}:{PORT}")
