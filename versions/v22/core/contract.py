...
class ContractV12(DataContract):
    """
    Implements the 43-feature context extraction for Pluribus V12 models (V22).

    37 features inherited from V21_auxhead unchanged, plus 6 new appended (not inserted -- every
    existing index 0-36 is stable): `opp_committed_this_hand_bb` per opponent slot (ctx[37:42],
    seat order matching the existing per-opponent block) and `hero_committed_this_hand_bb`
    (ctx[42]). See versions/v22/SPECS.md.
    ...
    def to_tensors(self, states, hero_actions: list = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # 3. Context (43 features) Sequence
        context_seq = [[0.0]*43 for _ in range(self.max_seq_len)]
