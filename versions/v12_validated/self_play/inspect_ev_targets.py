...
Run:  .venv/Scripts/python.exe -m versions.v12_validated.self_play.inspect_ev_targets
"""
import os
import sys
import random
...
def collect(n_hands=600, equity_sims=200):
    ...
    for h in range(n_hands):
        ...
        for dp in rec.decision_points:
            tevs = dp.get('target_evs', [0.0, 0.0, 0.0])
            realized = (rec.final_hero_profit + dp['committed_before']) / bb
            rows.append({
                'equity': dp['equity'],
                'street': dp['street'],
                'action': dp['action'],           # 0 fold, 1 call, 2 raise
                'ev': [e / bb for e in tevs],       # bb
...
def part1(rows):
    ...
    print("=" * 78)
    print(f"  {'equity':<12} {'n':>5} | {'EV_fold':>8} {'EV_call':>8} {'EV_raise':>9} | {'raise=argmax':>12}")
    for lo, hi in EQ_BUCKETS:
        ...
        er = sum(r['ev'][2] for r in b) / len(b)
        raise_arg = sum(1 for r in b if r['ev'][2] >= r['ev'][0] and r['ev'][2] >= r['ev'][1]) / len(b)
        print(f"  {f'{lo:.1f}-{hi:.1f}':<12} {len(b):>5} | {ef:>8.2f} {ec:>8.2f} {er:>9.2f} | {raise_arg*100:>10.1f} %")
...
def part2(rows):
    ...
    if not weak_raises:
        ...
        return
    mean_target = sum(r['ev'][2] for r in weak_raises) / len(weak_raises)
