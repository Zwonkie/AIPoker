...
Run:  .venv/Scripts/python.exe -m versions.v12_validated.self_play.overfit_sanity
"""
import os
import sys
import random
...
from versions.v12_validated.self_play.train import vectorize_hand_samples
...
def synthetic_targets(equity):
    """Clean monotonic EV as a function of equity only: fold=0, call/raise ramp with
    equity (raise steeper). Because equity is a model INPUT, this is trivially fittable
    for a healthy network -- that is exactly the point."""
    fold_ev = torch.zeros_like(equity)
    call_ev = 20.0 * (equity - 0.5)
    raise_ev = 30.0 * (equity - 0.45)
    return torch.stack([fold_ev, call_ev, raise_ev], dim=-1)
