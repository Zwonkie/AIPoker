...
Run:  .venv/Scripts/python.exe -m versions.v12_validated.self_play.inspect_policy_vs_target
"""
import os
import sys
import math
...
import versions.v12_validated.self_play.train as T
from versions.v12_validated.core.model import PokerEVModelV4
from versions.v12_validated.core.manifest import MANIFEST
from versions.v12_validated.self_play.simulator import SixMaxSimulator
...
def part_a(model, device, n_hands=500, max_seq_len=20):
    ...
    for h in range(n_hands):
        ...
        for i, dp in enumerate(dps):
            ...
            key = (street, bidx)
            a = agg.setdefault(key, {'n': 0, 'm': [0, 0, 0], 't': [0, 0, 0], 'kl': 0.0,
                                     'mH': 0.0, 'tH': 0.0})
            mp = model_pi_seq[t]
            tp = target_pi_seq[t]
            a['n'] += 1
    ...
    print("=" * 100)
    print("  PART A: MODEL policy vs TRAINING TARGET on live states  (F/C/R = fold/call/raise)")
    print(f"  target = {T.POLICY_TARGET_SOURCE} regret, temp={T.POLICY_TARGET_TEMP}")
    print("=" * 100)
    hdr = f"  {'street':<8} {'equity':<10} {'n':>5} | {'MODEL F/C/R':>22} | {'TARGET F/C/R':>22} | {'KL':>5} {'Hm':>5} {'Ht':>5}"
    print(hdr)
    for street in ['preflop', 'flop', 'turn', 'river']:
        for bidx, (lo, hi) in enumerate(EQ_BUCKETS):
            ...
            m = [x / n for x in a['m']]
            t = [x / n for x in a['t']]
            print(f"  {street:<8} {f'{lo:.1f}-{hi:.1f}':<10} {n:>5} | "
                  f"{m[0]:>6.2f}/{m[1]:>5.2f}/{m[2]:>5.2f}      | "
                  f"{t[0]:>6.2f}/{t[1]:>5.2f}/{t[2]:>5.2f}      | "
...
def part_b(model, device):
    ...
    from versions.v12_validated.core.contract import ContractV12
    bridge = ContractV12(max_seq_len=20)
    print("\n" + "=" * 100)
    print("  PART B: POSTFLOP sensitivity -- model policy on fixed spots (facing a 20 into 40 pot)")
    print("=" * 100)
    print(f"  {'label':<34} {'equity':>6} | {'P(fold)':>7} {'P(call)':>7} {'P(raise)':>8} | {'act':>5} || {'Q(fold/call/raise)':>26} | Qarg")
    spots = [
    ...
    for label, hole, board, street, eq in spots:
        ...
        act = "RAISE" if probs[2] >= probs[1] and probs[2] >= probs[0] else "CALL" if probs[1] >= probs[0] else "FOLD"
        qarg = "RAISE" if q[2] >= q[1] and q[2] >= q[0] else "CALL" if q[1] >= q[0] else "FOLD"
        print(f"  {label:<34} {eq:>6.2f} | {probs[0]:>7.2f} {probs[1]:>7.2f} {probs[2]:>8.2f} | {act:>5} || "
              f"{q[0]:>7.2f}/{q[1]:>7.2f}/{q[2]:>7.2f} | {qarg}")
...
def part_c(model, device):
    ...
    print("=" * 100)
    print(f"  {'equity_in':>9} {'ctx_eq_slot':>12} | {'Q(fold/call/raise)':>26} | {'P(fold/call/raise)':>22}")
    for eq in [0.10, 0.30, 0.50, 0.70, 0.90]:
        ...
        print(f"  {eq:>9.2f} {ctx_eq:>12.3f} | {q[0]:>7.2f}/{q[1]:>7.2f}/{q[2]:>7.2f} | "
