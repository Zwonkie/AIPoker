...
from versions.v41.core.model import PokerEVModelV4
from versions.v41.self_play.simulator import SixMaxSimulator, OPPONENT_POOL_STYLES, OPPONENT_POOL_WEIGHTS, STYLE_SLOT
from versions.v41.core.manifest import MANIFEST
from versions.v41.self_play.opponents import build_opponent_pool, describe_pool_entry
...
from versions.v41.core.contract import (
...
# [V29, BET-1/STACK-1] ALLIN-specific critic-consistency veto margin (bb units, same scale as the
# critic's own Q-value outputs) -- see regret_match_policy_torch's own docstring for the full
# derivation + calibration. 0.0 disables (no-op), same convention as every other training-loop knob.
CRITIC_CONSISTENCY_MARGIN = 0.0
...
def regret_match_policy_torch(action_values, equity=None, baseline_mode='mean', free_check_mask=None,
                              critic_consistency_margin=0.0):
    ...
    [V40, STACK-3] The dominance comparison below is now taken over the NON-FOLD alternatives only,
    so the veto can never fire merely because FOLD outranks ALLIN -- see the inline comment at the
    filter itself for why this is a provable no-op under baseline_mode='fold' and why it is kept
    anyway.

    [V29, BET-1/STACK-1] `critic_consistency_margin`: an ALLIN-specific veto, applied AFTER the
    usual fold-relative regret matching below. Motivated by a real (if narrower-than-hoped)
    gap: plain fold-relative regret matching gives ALLIN real probability mass any time it beats
    the ~0 fold baseline, even in states where it's clearly dominated by a smaller raise the critic
    itself ranks higher -- regret-vs-fold alone never compares actions against each OTHER, only
    against fold. Calibrated (scratchpad/calibrate_critic_consistency.py, against the frozen V28
    checkpoint's own raw Q-values across `deep_stack_ood_guard`'s eq x stack grid) BEFORE writing
    this: at eq=0.43 (all 5 stack depths), ALLIN's Q (0.16-1.01) clears the fold baseline but sits
    clearly below RAISE_POT's Q (1.29-1.55) -- exactly the spurious-weight case this fixes. But at
    eq=0.48/0.55 (the check's OTHER 10 failing cells), ALLIN's Q is the critic's own genuine max
    (e.g. 4.16 vs RAISE_POT's 3.51 at eq=0.55/stack=25) -- there this filter is a correct no-op,
    since ALLIN isn't actually dominated; that half of the check's footprint needs the critic
    itself to change (see `risk_aversion_coefficient`'s bump alongside this in versions/v29/SPECS.md
    -- NOT re-litigated here). Documented honestly as a PARTIAL fix for exactly this reason.
    Applied ONLY to ALLIN (not a general all-pairs dominance rule): an early all-pairs version of
    this filter, tested against the same calibration data, collapsed legitimate raise-size mixing
    (e.g. call/raise_33/raise_66/raise_pot's own intentional spread at eq=0.35/stack=40) down to a
    single surviving action in 19-23 of 25 grid cells even at a lenient margin -- a much bigger,
    undesired side effect on `action_diversity`/[BET-2]'s sizing mix than the targeted ALLIN fix
    was meant to have. ALLIN is always the LAST action (index -1) in this codebase's fixed 6-action
    contract (fold/call/raise_33/raise_66/raise_pot/allin), so a hardcoded index is safe here (same
    assumption `free_check_mask`'s call sites and every sized-model engine already make).
    Default 0.0 (no-op) for any caller that doesn't opt in, same pattern as every other
    training-loop knob (`risk_aversion_coefficient`, `policy_tightness_bb`).
    ...
    if critic_consistency_margin > 0.0:
        # [V40, STACK-3] Rescoped: the dominance comparison now runs over the NON-FOLD alternatives
        # (`1:-1`) instead of every non-ALLIN action (`:-1`), so the veto can never fire merely
        # because FOLD outranks ALLIN. Per the Fable review's own suggestion for finding #3.
        # HONEST NOTE, verified against both call sites (both pass baseline_mode='fold'): under the
        # fold baseline this is a PROVABLE NO-OP. It only changes `best_non_allin` when FOLD is the
        # argmax, and in exactly that case `regrets[..., -1]` is already clamped to 0 by
        # `(values - values[..., 0:1]).clamp(min=0.0)` above, so the veto had nothing left to zero.
        # Kept because it is correct-by-construction if baseline_mode='mean' is ever used here (where
        # the baseline is NOT fold and ALLIN can carry regret while FOLD outranks it), and because
        # the review's stated feedback-loop mechanism ("veto -> policy never jams -> ALLIN Q stays
        # low -> veto keeps firing") therefore does NOT hold as written -- the ALLIN Q head is
        # trained from the counterfactual target at every visited state regardless of what the policy
        # sampled. The genuinely open half of that finding (margin 0.15 possibly below critic noise;
        # four risk-dampeners -- variance penalty, realization discount, this veto, TARGET_CLIP_BB
        # -- stacked with no joint calibration) is a CALIBRATION question, deliberately not changed
        # blind here. See .agents/skills/OFK/references/fable-review-resolution-log.md.
        best_non_allin = values[..., 1:-1].max(dim=-1, keepdim=True).values
        allin_dominated = (best_non_allin - values[..., -1:]) > critic_consistency_margin
        regrets = regrets.clone()
        regrets[..., -1:] = torch.where(allin_dominated, torch.zeros_like(regrets[..., -1:]), regrets[..., -1:])
...
def run_intermediate_sensitivity_check(model, step_label, device):
    ...
    from versions.v41.core.contract import ContractV12, preflop_hand_strength
...
def run_training(personality, num_hands=100000, batch_size=256, epochs_per_batch=3,
    ...
    global CRITIC_CONSISTENCY_MARGIN
    CRITIC_CONSISTENCY_MARGIN = critic_consistency_margin  # [V29] ALLIN-specific regret-match veto
    ...
    from versions.v41.self_play.telemetry import TrainingTelemetry
    ...
    if range_aware_equity:
        from versions.v41.self_play.simulator import _get_preflop_ranked
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        if hands_done >= ACTOR_CRITIC_CUTOVER_HANDS:
                            ...
                            policy_target = regret_match_policy_torch(
                                preds.detach(), equity=b_c[:, :, 3], baseline_mode='fold',
                                free_check_mask=(b_c[:, :, 9] <= 1e-6),
                                critic_consistency_margin=CRITIC_CONSISTENCY_MARGIN)
                        else:
                            policy_target = b_pol
            ...
            with torch.no_grad():
                for b_h, b_b, b_c, b_a, b_sa, b_mc, b_m, b_bluff, b_str, b_eq, b_w, b_pol in val_loader:
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        if hands_done >= ACTOR_CRITIC_CUTOVER_HANDS:
                            policy_target = regret_match_policy_torch(
                                preds, equity=b_c[:, :, 3], baseline_mode='fold',
                                free_check_mask=(b_c[:, :, 9] <= 1e-6),
                                critic_consistency_margin=CRITIC_CONSISTENCY_MARGIN)
                        else:
                            policy_target = b_pol
...
if __name__ == '__main__':
    ...
    critic_consistency_margin = float(t_cfg.get('critic_consistency_margin', 0.0))  # [V29, BET-1/STACK-1] see regret_match_policy_torch
    ...
    print(f"  Critic consist.: {critic_consistency_margin}{'  (OFF)' if critic_consistency_margin == 0 else '  (vetoes ALLIN in actor target when dominated by margin, see regret_match_policy_torch)'}")
