...
from versions.v48.self_play.opponent_bots import (TAG, LAG, NIT, CALLING_STATION,
                                                  STYLE_SHIFT_SCALE, sample_raise_fraction)
from versions.v48.self_play.opponents import HeuristicOpponent, build_opponent_pool
from versions.v48.core.contract import preflop_hand_strength, effective_contested_field
...
class SixMaxSimulator:
    ...
    def __init__(self, bb_size=10.0, equity_sims=200, hero_personality='main',
        ...
        # [V48, Change 0] GENERALIZED chip-identity collapse: group raise buckets by resolved
        # chip amount (min-raise floor + stack clamp), duplicates take the canonical's exact
        # EV and zero actor-target regret (~73% of curriculum-weighted preflop raise decisions
        # carry a non-shove chip-identity -- measured by probe_minraise_aliasing.py). False =
        # V47's all-in-only behavior. Serve mirror: engine flag `collapse_aliased_buckets`.
        self.collapse_by_chips_all = True
        # [V48, Change 1] Table seat-count curriculum: list of [n_seats, weight] (3..6).
        # None/empty -> always 6 (verify-mode / prior-version compatibility). Set from config
        # by train.py; Change 2's joint (seat_count x depth) rows take precedence when set.
        self.table_size_mix = None
        # [V48, Change 2] [n_seats, lo_bb, hi_bb, weight] rows, measured from the live corpus.
        self.table_stack_joint_mix = None
        self._joint_depth_bb = None
    ...
    def _get_starting_stack(self, current_hand):
        ...
        # [V48, Change 2] JOINT seat-count x depth curriculum: when table_stack_joint_mix is
        # active, _sample_table_size already drew this hand's (n_seats, depth) row and stashed
        # the depth here -- consume it so seat count and depth stay CORRELATED the way the
        # measured DoN life-cycle is (6-handed deep early, 4-handed short at the bubble).
        _jd = getattr(self, '_joint_depth_bb', None)
        if _jd is not None:
            self._joint_depth_bb = None
            return round(_jd) * self.bb_size
    ...
    def _build_query_board_state(self, hand_cards, equity, pot_size, call_amount, hero_stack,
        ...
        # [V48, Change 1] Button-relative position over the PRESENT ring (compressed map),
        # matching the live path's occupied-ring convention (core/table_state.py, V42 #12-CE).
        # Falls back to the 6-ring offset when the ring isn't supplied (diagnostic call sites,
        # verify-mode full tables -- identical result there since the ring is all six seats).
        _ring = table_state_dict.get('present_ring') if table_state_dict else None
        if _ring and actor_seat in _ring and button_seat in _ring:
            actor_position = (_ring.index(actor_seat) - _ring.index(button_seat)) % len(_ring)
        else:
    ...
    def _query_model_decide(self, model, hand_cards, equity, pot_size, call_amount, hero_stack, num_opponents, table_state_dict=None, model_state_history=None, hero_actions_history=None, sample=True):
        ...
        """
        from versions.v48.core.contract import ContractV12
    ...
    def _opponent_raise_fraction(self, decision, agent):
        ...
        # [V48, Change 1b] street-split fitted repertoires (see opponent_bots.py provenance).
        return sample_raise_fraction(style, street_idx=getattr(self, '_betting_street_idx', None))
    ...
    def _mc_target_evs_sized(self, hero_cards, pot, to_call, hero_stack, street_idx,
        ...
        for j, (frac, raise_size, raise_increment, size_pot_odds, is_allin) in enumerate(size_infos):
            ...
            evs.append(raw_ev)
            aliased.append(False)   # finalized below by the chip-identity grouping
            if frac is None:
                allin_bucket_j = j

        # [V48, Change 0] GENERALIZED chip-identity collapse -- supersedes V47's all-in-only
        # rule (kept below as the fallback), which is the shove-group special case. Group raise
        # buckets by RESOLVED chip amount at this node: the legal min-raise floor makes
        # raise_33/66/pot chip-identical on ~73% of curriculum-weighted preflop decisions
        # (measured, probe_minraise_aliasing.py), far beyond the 2.4% all-in case -- and
        # duplicated buckets TRIPLE-COUNT one physical action's regret in the actor-target
        # normalization. Canonical per group: the ALLIN bucket for the shove group, else the
        # LOWEST-INDEX member. Duplicates take the canonical's EXACT EV (residual MC noise
        # cannot train different targets for one physical action) and aliased=True (zero
        # actor-target regret via train.py's masks -- unchanged, they are generic over these
        # flags). Serve-side mirror: core/decision.py keeps the SAME canonical per group,
        # gated on the engine flag `collapse_aliased_buckets` (train≡serve pair; V47's
        # narrower `collapse_aliased_allin` still serves the shove-only case).
        if getattr(self, 'collapse_by_chips_all', False):
            groups = {}
            for j, info in enumerate(size_infos):
                groups.setdefault(round(info[1], 6), []).append(j)
            for js in groups.values():
                if len(js) < 2:
                    continue
                canon = allin_bucket_j if allin_bucket_j in js else js[0]
                for j in js:
                    if j != canon:
                        evs[2 + j] = evs[2 + canon]
                        aliased[j] = True
        elif getattr(self, 'allin_by_chips', False) and allin_bucket_j is not None:
            # [V47, Change 2 / M9] all-in-only fallback: a chip-identical clamped bucket IS
            # the shove -- copy the canonical ALLIN bucket's EV over each clamped bucket.
            for j, info in enumerate(size_infos):
                if (j != allin_bucket_j and info[0] is not None
                        and info[1] >= hero_stack - 1e-9):
    ...
    def _sample_table_size(self):
        """[V48, Change 1/2] Seat count for this hand. Precedence:
        1. `table_stack_joint_mix` ([n_seats, lo_bb, hi_bb, weight] rows -- the MEASURED joint
           DoN life-cycle, live2/historydb corpus): draws (n, depth) together and stashes the
           depth for _get_starting_stack, keeping the seat-depth CORRELATION real.
        2. `table_size_mix` ([n, weight] rows -- seat count only, independent depth).
        3. Neither set -> 6 (verify-mode / prior-version compatibility)."""
        joint = getattr(self, 'table_stack_joint_mix', None)
        if joint:
            row = random.choices(joint, weights=[float(r[3]) for r in joint], k=1)[0]
            n, lo, hi = int(row[0]), float(row[1]), float(row[2])
            self._joint_depth_bb = random.uniform(lo, hi)
            return max(2, min(6, n))
        mix = getattr(self, 'table_size_mix', None)
        if not mix:
            return 6
        sizes = [int(m[0]) for m in mix]
        weights = [float(m[1]) for m in mix]
        n = random.choices(sizes, weights=weights, k=1)[0]
        return max(2, min(6, n))
    ...
    def simulate_hand(self, current_hand=0):
        ...
        # 1. Initialize Seats
        # [V48, Change 1] TRUE N-handed dealing: sample the table's seat count from the
        # curriculum and mark ABSENT seats before any geometry is derived. Hero (seat 0) is
        # always present; absent seats get no cards, no stack, no blind, no position -- they
        # are NOT "pre-folded players" (no dead stacks parked behind them). The diagnostic
        # `live_players` cap and the >40k pre-fold curriculum still operate on the PRESENT
        # ring below: a 6-seat table where 3 fold is a DIFFERENT node than a 3-seat table
        # (different dead money, same live count) and both occur in real DoN play.
        present = [True] * 6
        n_seats = self._sample_table_size()
        if n_seats < 6:
            for _s in random.sample(range(1, 6), 6 - n_seats):
                present[_s] = False
        present_ring = [s for s in range(6) if present[s]]

        button_seat = random.choice(present_ring)
        ...
        # never get to act -- hero learned that attacking blinds prints money because the blind
        # literally cannot fight back. A seat that posts a blind is always a seat that can act.
        # [V48, Change 1] The orbit now walks the PRESENT ring: a real 4-handed table's blinds
        # hit ~50% more often and there is no 6-ring gap between button and blinds.
        sb_seat = _next_present(button_seat)
        bb_seat = _next_present(sb_seat)
        # Opponent seats the pre-fold is allowed to remove (never a blind, never absent; seat 0
        # is hero).
        prefold_candidates = [s for s in range(1, 6) if present[s] and s not in (sb_seat, bb_seat)]
        ...
        # [V48, Change 1] Absent seats hold no chips -- they must encode like live's unoccupied
        # slots (inactive, stack 0), not like folded players with dead stacks behind them.
        for _s in range(1, 6):
            if not present[_s]:
                stacks[_s] = 0.0
        ...
        folded = [not present[s] for s in range(6)]   # [V48, Change 1] absent -> never acts
        ...
        if 0 < self.live_players < 6:
            ...
            remaining = [s for s in range(1, 6) if present[s] and s not in live_opp_seats]
            ...
            for s in range(1, 6):
                if present[s] and s not in live_opp_seats:
                    active[s] = False
                    folded[s] = True
            ...
            while not betting_ended:
                if not folded[current_actor] and stacks[current_actor] > 0:
                    ...
                    table_state = {
                        ...
                        # [V48, Change 1] the PRESENT ring, so every query's button-relative
                        # position uses the compressed N-handed map (live occupied-ring
                        # convention), not the raw 6-ring offset.
                        "present_ring": list(present_ring),
                        "hand_strength": hs,
                    ...
                    if current_actor == 0:  # Hero
                        ...
                        if decision == 'fold':
                            ...
                            # [V48, Change 1b] street stash (not a signature change -- the C2
                            # InstrumentedSim subclass overrides _opponent_raise_fraction with
                            # the V47 signature) so heuristic seats sample from the STREET-SPLIT
                            # fitted tables: measured preflop raises are big/jam-heavy, measured
                            # postflop bets are small (population fit, hero excluded).
                            self._betting_street_idx = street_idx
