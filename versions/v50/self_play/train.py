...
class Tee:
    """[V44] Writes to any number of files plus the real stdout.

    Two requirements that look contradictory but are not:
      * The dashboard watcher (tools/training_monitor/start_dashboard.ps1) polls ONE fixed path,
        <repo_root>/active_training.log, every 5s. That fixed name is a deliberate CONTRACT --
        documented in .agents/AGENTS.md and enforced by
        tools/training_monitor/check_telemetry_contract.py -- so the watcher never has to guess
        which version is training.
      * A run must not destroy the previous run's log (V43's run overwrote V41's).

    V43 satisfied the second by renaming the file per version, which silently broke the first:
    training ran outside the dashboard's view and it kept showing the PREVIOUS run's finished
    numbers, looking merely stale rather than disconnected. Writing both files satisfies both,
    which is what the second argument is for.
    """
...
from versions.v50.core.model import PokerEVModelV4
from versions.v50.self_play.simulator import SixMaxSimulator, OPPONENT_POOL_STYLES, OPPONENT_POOL_WEIGHTS, STYLE_SLOT
from versions.v50.core.manifest import MANIFEST
from versions.v50.self_play.opponents import build_opponent_pool, describe_pool_entry
...
from versions.v50.core.contract import (
...
# --- Q-target shaping constants ---
# TARGET_CLIP_BB: hard clip on the go-forward MC target, in bb.
#
# [V43] 40.0 -> 100.0, matching `core/contract.py`'s STACK_CEIL_BB and the top of
# `stack_depth_mix`. This is review finding T-M5: the clip sat at 40bb while the curriculum
# reached 100bb and the contract can represent stacks to 100bb, so a 100bb stack-off and a 40bb
# loss produced IDENTICAL targets -- the value scale and the input scale disagreed.
#
# Measured before changing it (600 hands, versions/v43 simulator):
#   |realized go-forward return| > 40bb  -> 23.4% of decision points   (the clip was binding hard)
#   |realized go-forward return| > 100bb ->  5.9%   p95 = 102bb, max = 167bb
# So at 40 the clip truncated nearly a quarter of the realized channel; at 100 it truncates ~6%,
# and what remains are genuine multiway stack-offs where hero wins more than one covering stack.
#
# Why raising it is gradient-safe: the critic's loss is `nn.HuberLoss(delta=2.0)` (see
# run_training), so per-sample gradient magnitude saturates at delta for any error beyond 2bb --
# a 100bb target does not produce a 2.5x larger gradient than a 40bb one. The clip therefore
# controls BIAS (what value the critic converges to), not stability. The original 40 was chosen to
# damp a fat right tail that biased "enter" Q-values upward; that bias source is now addressed at
# the root instead (see the realization-discount note below and versions/v43/SPECS.md).
#
# Residual, deliberately accepted: ~6% of returns still clip. Hero at 100bb multiway can win more
# than 100bb, but the CONTRACT cannot represent a stack above STACK_CEIL_BB=100, so clipping the
# target at the same ceiling keeps the two scales consistent rather than teaching the critic
# magnitudes its own inputs cannot distinguish.
TARGET_CLIP_BB = 100.0
...
# [V43] REMOVED: the realization discount (`POLICY_TIGHTNESS_BB` / `POLICY_TIGHTNESS_PIVOT`, V12)
# and the ALLIN critic-consistency veto (`CRITIC_CONSISTENCY_MARGIN`, V29).
#
# Both were corrective priors imposed ON TOP of the training target to suppress behaviour the
# target itself was producing for reasons that have since been fixed at the source. The discount
# subtracted a flat bb amount from every voluntary action below eq 0.45 because "the all-in-equity
# counterfactual overvalues speculative entries"; the veto zeroed ALLIN's regret whenever another
# action's Q beat it by 0.15bb. Between them they hard-coded two answers the model is supposed to
# LEARN -- how wide to enter, and when to jam.
#
# The root causes they were compensating for have been addressed since they were calibrated:
# V40 fixed the betting round ending on any check (postflop nodes did not exist in training data
# at all) and CALL's exemption from both the variance penalty and the multi-street continuation
# credit; V41 fixed dead blinds, NN opponents playing a degraded self, symmetric stacks, the
# min-raise rule, and the [OPP-7] encoding that hid hero from every opponent query.
#
# Measured before removing them (versions/v43/self_play/ablate_dampeners.py, 349-cell eq x stack
# grid, target space):
#   realization discount -- no effect on entry rate at depth (0.92->0.88 with, 0.93->0.88 without)
#                           but real at its design point: entry rate at eq <= 0.35 is 0.59 with,
#                           0.79 without. It IS restraining weak entries. Removing it is a
#                           deliberate decision to let correct inputs teach that restraint instead
#                           of imposing it -- EXPECT entry range / VPIP to widen, and check it.
#   ALLIN veto           -- near-inert: ALLIN's target share 0.37-0.43 with, 0.41-0.43 without;
#                           entry rate at eq <= 0.35 identical (0.59 vs 0.60). Consistent with V40's
#                           finding that its rescope was a provable no-op under the fold baseline.
#                           (That measurement covers the PRE-cutover path; post-cutover the veto
#                           applied to the critic's Q, which this harness does not exercise.)
#
# The variance penalty (`risk_aversion_coefficient`) is KEPT: the same ablation showed the [BET-1]
# pathology it exists to suppress is NOT gone at the source -- removing it triples the
# ALLIN-vs-next-best gap's growth with stack depth (+3.24 -> +10.58 bb across 5->100bb).
#
# V41 remains the rollback with both knobs intact. See versions/v43/SPECS.md.
...
def regret_match_policy(action_values, aliased=None):
    ...
    [V47, Change 2 / M9] `aliased`: optional length-K bool list, True for a raise bucket whose
    chips are identical to the shove at this node. Aliased buckets get ZERO regret so their
    (equal-by-construction) EV cannot split probability mass four ways across four names for one
    physical action -- the mass concentrates on the canonical ALLIN bucket instead. The uniform
    fallback also excludes them.
    """
    mean_v = sum(action_values) / len(action_values)
    regrets = [max(v - mean_v, 0.0) for v in action_values]
    if aliased:
        regrets = [0.0 if a else r for r, a in zip(regrets, aliased)]
    total = sum(regrets)
    if total <= 1e-9:
        if aliased and any(aliased):
            live = [0.0 if a else 1.0 for a in aliased]
            n_live = sum(live)
            if n_live > 0:
                return [l / n_live for l in live]
        n = len(action_values)
        return [1.0 / n] * n
...
def regret_match_policy_torch(action_values, baseline_mode='mean', free_check_mask=None,
                              aliased_mask=None):
    ...
    [V43] Two corrective priors were REMOVED from this function -- the `equity`-driven realization
    discount (V12) and the `critic_consistency_margin` ALLIN veto (V29). Both imposed an answer the
    model is meant to learn (how wide to enter; when to jam) to compensate for target defects that
    V40/V41 have since fixed at the source. See the constants block at the top of this file for the
    measurements taken before removing each, and versions/v43/SPECS.md for the rationale. V41 is the
    rollback if entry range or jam frequency regresses without them.

    [V27, VAL-3 fix] `free_check_mask`: optional [B, T] bool tensor, True where call_amount==0 for
    that timestep (a free check). With baseline_mode='fold', every action's regret is measured
    against the model's OWN Q_fold output -- whose training target is a hardcoded constant 0.0 in
    EVERY state (`_mc_target_evs_sized`'s evs[0]). Q_call is ALSO naturally near zero exactly when
    equity is low and checking is free (call EV = equity*pot - 0). Critic noise nudging Q_fold past
    every other action in that corner trips the "nothing beats folding" fallback below and hands
    the actor a literal FOLD=1.0 label for a state where folding a free option is never correct --
    the exact corner `free_check_low_fold` flags (max raw P(fold)=1.000 when call_bb=0). Every OTHER
    place in this codebase already masks fold to zero when call_amount==0 (`_query_model_decide`'s
    self-play action selection, `core/decision.py`'s live inference) -- this was the one remaining
    gap, in the training TARGET itself. Applied by zeroing fold and renormalizing over the
    remaining actions, same recipe as those other two call sites.
    ...
    action_values: [B, T, K] tensor (expected DETACHED by the caller -- this function does not
        ...
        through this path).
    free_check_mask: optional [B, T] bool tensor (True where call_amount==0) -- see [V27] note above.
    aliased_mask: optional [B, T, K] bool tensor ([V47, Change 2 / M9] -- True for a raise bucket
        whose chips equal the shove at that node). Aliased buckets get zero regret so equal EVs
        can't split actor mass four ways across one physical action; the fold-mode fallback
        (fold=1.0) is unaffected since FOLD is never aliased.
    ...
    if aliased_mask is not None:
        regrets = regrets.masked_fill(aliased_mask, 0.0)
    ...
    result = torch.where(total > 1e-9, matched, fallback)
    if free_check_mask is not None:
        # [V27, VAL-3 fix] Zero fold and renormalize over the remaining actions wherever checking
        # is free -- same recipe _query_model_decide (self-play) and core/decision.py (live) already
        # apply at their own decision points; this is the one place upstream of both that didn't.
        zeroed = result.clone()
        zeroed[..., 0] = 0.0
        renorm = zeroed.sum(dim=-1, keepdim=True)
        # Degenerate case: the ORIGINAL result was already all-fold (renorm==0 after zeroing it) --
        # e.g. the fold-baseline fallback above. Nothing left to redistribute proportionally, so
        # default to CALL, exactly like simulator.py:640's own free-check fallback (`else
        # torch.tensor([0.0, 1.0, 0.0], ...)`) for the same situation at inference/self-play time.
        call_default = torch.zeros_like(result)
        call_default[..., 1] = 1.0
        redistributed = torch.where(renorm > 1e-9, zeroed / renorm.clamp(min=1e-9), call_default)
        fc = free_check_mask.unsqueeze(-1)
        result = torch.where(fc, redistributed, result)
    return result
...
def vectorize_hand_samples(record, max_seq_len=20, compute_policy_target=True):
    ...
    context_seq = [[0.0]*54 for _ in range(max_seq_len)]
    ...
    # [V47, Change 2 / M9] per-step, per-action "this raise bucket is chip-identical to the
    # shove" flags (recorded by _mc_target_evs_sized; fold/call always 0) -- consumed by the
    # actor-target computation here (dataset-time) and in the training loop (critic-time).
    aliased_seq = [[0.0]*K for _ in range(max_seq_len)]
    ...
    for i, dp in enumerate(dps):
        ...
        # [V44] Normalize by the EFFECTIVE contested field the simulator measured this decision's
        # equity against, not the nominal opponent count -- see contract.equity_edge_feature. This
        # line and ContractV12.to_tensors are independent implementations of the SAME feature, so
        # they must change together or the gradient tensors and every inference path disagree about
        # what ctx[35] means. Same 0.0 -> nominal fallback as the contract's, so a decision point
        # recorded before this field existed still vectorizes exactly as V43 did.
        _eff = float(dp.get('effective_field', 0.0) or 0.0)
        ctx.append(equity_edge_feature(dp['equity'], _eff if _eff > 0 else active_opps_count))
        ctx.append(float(dp.get('hand_strength', 0.5)))

        # [V22] Six new appended features (indices 37-42 -- everything above is untouched
        # V21_auxhead). Mirrors contract.py's to_tensors exactly (same scaled_stack_bb helper,
        # same seat order, same 0.0 defaults) -- see versions/v22/SPECS.md.
        opponents_committed = dp.get('opponents_committed') or [0.0] * 5
        for j in range(5):
            ctx.append(scaled_stack_bb(opponents_committed[j], bb))
        ctx.append(scaled_stack_bb(dp.get('committed_before', 0.0), bb))

        # [V23] One new appended feature (index 43 -- everything above is untouched V22). Mirrors
        # contract.py's to_tensors exactly (same bucketing, same /2.0 normalization) -- see
        # versions/v23/SPECS.md.
        ctx.append(float(min(2, dp.get('raise_count_before', 0))) / 2.0)

        # [V29] Ten new appended features (indices 44-53 -- everything above is untouched V23-V28).
        # Mirrors contract.py's to_tensors exactly (same seat order, same False/0.0 defaults) --
        # [OPP-2], see versions/v29/SPECS.md.
        opponents_raised_this_hand = dp.get('opponents_raised_this_hand') or [False] * 5
        opponents_raised_this_street = dp.get('opponents_raised_this_street') or [False] * 5
        for j in range(5):
            ctx.append(1.0 if opponents_raised_this_hand[j] else 0.0)
        for j in range(5):
            ctx.append(1.0 if opponents_raised_this_street[j] else 0.0)
        ...
        # [V47, Change 2 / M9] full-K alias flags for this step: [fold, call] never alias; the
        # recorded per-raise-bucket flags fill the rest (padded False for records without them).
        _al_flags = list(dp.get('allin_aliased') or [])
        al_full = ([False, False] + [bool(x) for x in _al_flags] + [False] * K)[:K]
        aliased_seq[idx] = [1.0 if a else 0.0 for a in al_full]
        ...
        if compute_policy_target:
            ...
            if POLICY_TARGET_SOURCE == 'counterfactual':
                # [V43] The realization discount that used to be applied here (and, identically,
                # inside regret_match_policy_torch for the post-cutover path) is REMOVED -- see the
                # constants block at the top of this file. The counterfactual EVs now reach the
                # actor target unmodified.
                p_evs = list(mc_evs)
                p_evs[0] = 0.0
            else:
                p_evs = t_evs
            policy_target_seq[idx] = sharpen_distribution(
                regret_match_policy(p_evs, aliased=al_full), POLICY_TARGET_TEMP)
    ...
    return [(hole_ints, board_seq, context_seq, action_seq, action_taken_seq, target_evs_seq, loss_mask, opp_bluff_seq, opp_strength_seq, self_equity_seq, target_w_seq, policy_target_seq, aliased_seq)]
...
def simulate_worker(current_hand, bb_size, equity_sims, num_hands, hero_personality,
                    ...
                    risk_aversion_coefficient=0.0, table_stack_joint_mix=None):
    ...
    # [V48, Change 1/2] Joint seat-count x depth curriculum (measured DoN life-cycle rows).
    # Unconditional assignment per the V28 policy-temperature lesson (never hasattr-guard).
    sim.table_stack_joint_mix = table_stack_joint_mix
    sim.disable_exploration = disable_exploration
    sim.range_aware_equity = range_aware_equity
    # [V28, BET-1] Risk/variance penalty coefficient for _mc_target_evs_sized -- see that method's
    # own docstring. Always assigned unconditionally (never hasattr-guarded): a prior bug
    # (policy_temperature, fixed 2026-07-15) showed that hasattr() on a freshly-constructed
    # SixMaxSimulator is always False for an attribute never pre-declared in __init__, silently
    # skipping a guarded assignment -- default 0.0 here means "no penalty" for any caller that
    # doesn't pass this through, matching every other version's implicit behavior.
    sim.risk_aversion_coefficient = risk_aversion_coefficient
...
def run_intermediate_sensitivity_check(model, step_label, device):
    ...
    from versions.v50.core.contract import ContractV12, preflop_hand_strength
...
def run_training(personality, num_hands=100000, batch_size=256, epochs_per_batch=3,
                 ...
                 range_aware_equity=False,
                 stack_depth_mix=None, risk_aversion_coefficient=0.0,
                 table_stack_joint_mix=None):
    ...
    #    fat-tailed; the same state then gets wildly varying targets each visit, the critic
    #    diverges, and the regret-matching actor collapses to uniform. Keep this ON. Set to a
    #    large number only to deliberately test instability. [V43] The VALUE moved 40 -> 100bb to
    #    match STACK_CEIL_BB and the curriculum's reach -- see the constant's own note. The clip
    #    stays; only its ceiling changed.
    ...
    #  * [V43] `policy_tightness_bb` and `critic_consistency_margin` are GONE -- both the args and
    #    their globals. They are not accepted-and-ignored: passing either is now a TypeError, so a
    #    stale config or call site fails loudly instead of silently training without a knob the
    #    caller believed was on.
    ...
    # [V47, Change 5 / M7] T_max = the number of scheduler.step() calls this run actually makes
    # (one per sim batch), so the cosine anneal COMPLETES at the planned end of the run. The old
    # hardcoded T_max=100 against ~50 batches per 100k-hand run meant the anneal never got past
    # mid-curve -- the LR floor was never reached in any run of this lineage.
    _planned_batches = max(1, -(-max(1, num_hands - initial_hands_done) // max(1, sim_batch_size)))
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=_planned_batches)
    ...
    if os.path.exists(checkpoint_path):
        ...
        try:
            ...
            # [V47, Change 5 / VAL-5] --resume_path used to restore WEIGHTS ONLY: fresh Adam
            # moments + a restarted cosine schedule silently changed the optimization dynamics of
            # every resumed run vs the equivalent uninterrupted one -- the prime [VAL-5] suspect.
            # Checkpoints now carry both (see the save_checkpoint calls below); restore them when
            # present, and say so LOUDLY either way. Restoring scheduler_state also restores the
            # ORIGINAL run's T_max/position, deliberately overriding the fresh one above --
            # a resumed run continues its own schedule, it does not start a new one.
            if resume_path and isinstance(checkpoint, dict):
                if 'optimizer_state' in checkpoint:
                    optimizer.load_state_dict(checkpoint['optimizer_state'])
                    print("Resumed optimizer state (Adam moments).")
                else:
                    print("NOTICE: resume checkpoint carries no optimizer_state -- Adam moments start FRESH.")
                if 'scheduler_state' in checkpoint:
                    scheduler.load_state_dict(checkpoint['scheduler_state'])
                    print("Resumed LR-scheduler state (cosine position + T_max).")
                else:
                    print("NOTICE: resume checkpoint carries no scheduler_state -- LR schedule starts FRESH.")
        except Exception as e:
    ...
    from versions.v50.self_play.telemetry import TrainingTelemetry
    ...
    if range_aware_equity:
        from versions.v50.self_play.simulator import _get_preflop_ranked
        print("  Building preflop range ranking (range-aware equity)...")
        _get_preflop_ranked()

    # [V41] `POKER_NUM_WORKERS` override. Windows + spawn + one CUDA-capable evaluator per worker
    # is the documented deadlock surface for this codebase (see OFK
    # windows-pytorch-multiprocessing-deadlock.md); being able to dial the pool down without
    # editing code is what makes that class of hang bisectable instead of guesswork.
    _env_workers = os.environ.get('POKER_NUM_WORKERS')
    num_workers = int(_env_workers) if _env_workers else min(os.cpu_count(), 8)
    active_workers = num_workers
    pool = Pool(active_workers)
    try:
        while hands_done < num_hands:
            ...
            if hands_done - run_training.last_diag >= mid_flight_diagnostics_interval:
                ...
                # [V47, Change 5 / VAL-5] every resumable checkpoint carries optimizer + scheduler
                # state so --resume_path restores the full optimization state, not weights only.
            ...
            if hands_done - run_training.last_dump >= checkpoint_dump_interval:
                ...
                save_checkpoint(model.state_dict(), dump_path, MANIFEST, hands_trained=hands_done,
                                optimizer_state=optimizer.state_dict(),   # [V47 VAL-5] resumable
                                scheduler_state=scheduler.state_dict())
            ...
            # Worker args
            hands_per_worker = max(1, batch_hands // active_workers)
            ...
            args = [
                (hands_done, 10.0, equity_sims, hands_per_worker, personality,
                 ...
                 stack_depth_mix, risk_aversion_coefficient, table_stack_joint_mix)
                for _ in range(active_workers)
            ]

            results = pool.starmap(simulate_worker, args)

            # Ramp-up complete after the first batch -- scale up to the full worker count.
            ...
            batch_records = []
            # [V47, Change 5 / M6] the LAST worker's records form this batch's HELD-OUT
            # validation stream (own simulator instance / RNG / opponent draws): they stay in
            # batch_records for hands accounting + telemetry, but never enter the train split.
            val_batch_records = []
            ...
            for _w_idx, (res, worker_hist, worker_extra, worker_exploitation) in enumerate(results):
                batch_records.extend(res)
                if len(results) >= 2 and _w_idx == len(results) - 1:
                    val_batch_records.extend(res)
            ...
            # Telemetry over EVERY simulated hand (train and val alike -- it describes the
            # simulation stream, not the training split).
            ...
            def _vectorize_records(records):
                """Records -> TensorDataset (or None if empty)."""
                cols = [[] for _ in range(13)]
                for rec in records:
                    for sample in vectorize_hand_samples(rec, compute_policy_target=_compute_pol):
                        for col, value in zip(cols, sample):
                            col.append(value)
                if not cols[0]:
                    return None
                return TensorDataset(
                    torch.tensor(cols[0], dtype=torch.long),      # hole
                    torch.tensor(cols[1], dtype=torch.long),      # board
                    torch.tensor(cols[2], dtype=torch.float32),   # ctx
                    torch.tensor(cols[3], dtype=torch.long),      # act history
                    torch.tensor(cols[4], dtype=torch.long),      # action taken
                    torch.tensor(cols[5], dtype=torch.float32),   # target evs
                    torch.tensor(cols[6], dtype=torch.float32),   # loss mask
                    torch.tensor(cols[7], dtype=torch.float32),   # opp bluff
                    torch.tensor(cols[8], dtype=torch.float32),   # opp strength
                    torch.tensor(cols[9], dtype=torch.float32),   # self equity
                    torch.tensor(cols[10], dtype=torch.float32),  # target weights
                    torch.tensor(cols[11], dtype=torch.float32),  # policy target
                    torch.tensor(cols[12], dtype=torch.float32),  # [V47 M9] aliased flags
                )

            # [V47, Change 5 / M6] HELD-OUT validation stream: val hands come from their own
            # dedicated simulator instances (the last worker's records -- independent deals,
            # opponent draws, and RNG stream, generated alongside the batch), not a random_split
            # of the same records the model then trains on for epochs_per_batch epochs. During
            # the single-worker ramp-up batch there is no second worker to hold out, so that one
            # batch falls back to a sequential 90/10 hand split.
            train_ds = _vectorize_records(batch_records if not val_batch_records else
                                          [r for r in batch_records if id(r) not in
                                           set(id(v) for v in val_batch_records)])
            val_ds = _vectorize_records(val_batch_records) if val_batch_records else None
            if train_ds is None:
                continue
            if val_ds is None:
                n = len(train_ds)
                ...
                train_ds, val_ds = Subset(train_ds, range(cut)), Subset(train_ds, range(cut, n))

            total_training_samples += len(train_ds) + len(val_ds)
            ...
            for epoch in range(epochs_per_batch):
                ...
                epoch_loss_eq = 0.0
                total_items = 0
                for b_h, b_b, b_c, b_a, b_sa, b_mc, b_m, b_bluff, b_str, b_eq, b_w, b_pol, b_al in train_loader:
                    b_al = b_al.to(device, non_blocking=True)   # [V47 M9] aliased-bucket flags
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        if hands_done >= ACTOR_CRITIC_CUTOVER_HANDS:
                            # [V27, VAL-3] b_c[:, :, 9] is the scaled call_amount feature -- <= 0
                            # means checking is free this step (see core/contract.py's ctx layout).
                            # [V47, Change 2 / M9] aliased_mask concentrates chip-identical raise
                            # buckets' actor mass onto the canonical ALLIN bucket.
                            policy_target = regret_match_policy_torch(
                                preds.detach(), baseline_mode='fold',
                                free_check_mask=(b_c[:, :, 9] <= 1e-6),
                                aliased_mask=(b_al > 0.5))
                        else:
                            policy_target = b_pol
            ...
            val_items = 0
            with torch.no_grad():
                for b_h, b_b, b_c, b_a, b_sa, b_mc, b_m, b_bluff, b_str, b_eq, b_w, b_pol, b_al in val_loader:
                    b_al = b_al.to(device)   # [V47 M9] aliased-bucket flags
                    b_h = b_h.to(device)
                    b_b = b_b.to(device)
                    b_c = b_c.to(device)
                    b_a = b_a.to(device)
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        if hands_done >= ACTOR_CRITIC_CUTOVER_HANDS:
                            policy_target = regret_match_policy_torch(
                                preds, baseline_mode='fold',
                                free_check_mask=(b_c[:, :, 9] <= 1e-6),
                                aliased_mask=(b_al > 0.5))   # [V47 M9] same mask as training
                        else:
                            policy_target = b_pol
                        log_policy = nn.functional.log_softmax(pred_policy, dim=-1)
                        loss_pi_step = -(policy_target * log_policy).sum(dim=-1) * b_m
...
if __name__ == '__main__':
    ...
    # Two log files, deliberately.
    #
    # `active_training.log` is a CONTRACT, not a convenience: the dashboard watcher
    # (tools/training_monitor/start_dashboard.ps1) polls that exact path every 5s so it never has
    # to guess which version is training. Documented in .agents/AGENTS.md, enforced by
    # tools/training_monitor/check_telemetry_contract.py -- RUN THAT CHECKER after scaffolding a
    # new version.
    #
    # [V43] made it per-version because a fixed 'w'-mode path meant each run overwrote the last
    # (V43's run destroyed V41's log). That fixed the clobbering and silently broke the contract:
    # training ran outside the dashboard's view, which looked like merely stale numbers rather than
    # a disconnected feed. Writing BOTH satisfies both -- fixed path for the watcher, per-version
    # archive named from this slice's own manifest so no version can clobber another's history.
    from versions.v50.core.manifest import MANIFEST as _M
    ...
    # [V48, Change 2] joint [[n_seats, lo_bb, hi_bb, w], ...] rows -- takes precedence in the sim.
    table_stack_joint_mix = c_cfg.get('table_stack_joint_mix', None)
    ...
    # [V43] Default 100.0 (was 40.0) -- matches STACK_CEIL_BB and the curriculum ceiling. See the
    # TARGET_CLIP_BB constant's own note for the measurement behind the change.
    target_clip_bb = float(t_cfg.get('target_clip_bb', 100.0))  # variance clip; keep ON
    ...
    risk_aversion_coefficient = float(t_cfg.get('risk_aversion_coefficient', 0.0))  # [V28, BET-1] see simulator.py's _mc_target_evs_sized

    # [V43] FAIL LOUD on a config that still sets a removed knob. Silently ignoring it would mean
    # training without a corrective the config's author believed was active -- precisely the class
    # of quiet degradation the Fable review kept finding (dead `past_model` attrs, dead
    # `CRITIC_ARGMAX_MODE`, stale version ladders). A removed knob must not be a no-op.
    _removed = [k for k in ('policy_tightness_bb', 'critic_consistency_margin') if k in t_cfg]
    if _removed:
        raise ValueError(
            f"config.yaml sets {_removed}, which V43 REMOVED (realization discount / ALLIN "
            f"critic-consistency veto). They are gone from the code, not defaulted to 0 -- delete "
            f"the keys, or run versions/v41 if you want them. See versions/v43/SPECS.md.")
    ...
    print(f"  Target clip:     {target_clip_bb} bb   (V43: 40 -> 100, matches STACK_CEIL_BB)")
    ...
    print(f"  Risk aversion:   {risk_aversion_coefficient}{'  (OFF -- no risk penalty)' if risk_aversion_coefficient == 0 else '  (penalizes sqrt(Var) per size, see simulator.py)'}")
    print(f"  Corrective priors: REMOVED in V43 -- no realization discount, no ALLIN veto "
          f"(entry width and jam frequency are learned, not imposed)")
