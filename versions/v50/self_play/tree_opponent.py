...
data, not a clustering bug. See versions/v28/SPECS.md for the full pipeline and this caveat.
...
# [V47, Change 1 / #6] The predicted size class now SURVIVES the interface boundary: each raise
# class maps to the simulator's own raise-bucket string ('raise_k', k indexing
# SixMaxSimulator.raise_fracs = [0.33, 0.66, 1.0, None]), which `_opponent_raise_fraction`
# executes through the shared `_raise_size_for_fraction`. Before V47 all four collapsed to a bare
# 'raise' and the simulator sized it 0.75x pot -- the model predicted 6 classes and the pipeline
# threw the sizing information away (see this module's own docstring, now historical).
_RAISE_CLASS_TO_BUCKET = {"raise_33": "raise_0", "raise_66": "raise_1",
                          "raise_pot": "raise_2", "allin": "raise_3"}
...
class TreeOpponent:
    ...
    def _predict_action(self, equity, pot_odds, street_idx, stack, num_opps):
        try:
            ...
            # [V47, Change 1] carry the size class through as a 'raise_k' bucket string.
            return _RAISE_CLASS_TO_BUCKET.get(action, action)
        except Exception:
            return None  # caller falls back to recording_bot

    def fold_prob(self, equity, pot_odds, street_idx, stack, num_opps):
        """[V47, Change 3 / M4] P(fold) from the SAME predicted distribution `_predict_action`
        samples from (proba[0], renormalized) -- the occupant-true counterfactual fold estimate
        for a Tree-backed seat. Returns None on any failure (caller falls back to the heuristic
        proxy)."""
        try:
            eq = self._clip(equity, "equity")
            po = self._clip(pot_odds, "pot_odds")
            ...
            proba = self.booster.predict(xgb.DMatrix(features))[0]
            return float(proba[0] / proba.sum())
        except Exception:
            return None
