...
class ContractV12(DataContract):
    """
    Implements the 44-feature context extraction for Pluribus V12 models (V23).

    43 features inherited from V22 unchanged, plus 1 new appended (not inserted -- every existing
    index 0-42 is stable): `pot_type` (ctx[43], bucketed 0=limped/unraised, 1=single-raised,
    2=3-bet+, normalized /2.0). See versions/v23/SPECS.md.
    ...
    def to_tensors(self, states, hero_actions: list = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # 3. Context (44 features) Sequence
        context_seq = [[0.0]*44 for _ in range(self.max_seq_len)]
