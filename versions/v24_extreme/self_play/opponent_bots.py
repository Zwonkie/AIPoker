...
class FuzzyPlayerArchetype:
    def __init__(
        ...
        # the raise-respect mechanism for now. See versions/v24/SPECS.md.
...
TAG = FuzzyPlayerArchetype(
    ...
    # [V24_EXTREME] Pushed to near-0 (was 0.20) -- deliberately NOT a calibrated value, maximizing
    # respect-application rate (~98%) to test whether ANY strength of this lever can move
    # action_diversity/deep_stack_ood_guard at all. See versions/v24_extreme/SPECS.md.
    base_bluff_perc=0.02,
)
...
LAG = FuzzyPlayerArchetype(
    ...
    # [V24_EXTREME] Pushed to near-0 (was 0.35) -- see TAG's own comment above.
    base_bluff_perc=0.02,
)
...
NIT = FuzzyPlayerArchetype(
    ...
    # [V24_EXTREME] Already near-0 in V24 (0.05); pushed slightly further for consistency.
    base_bluff_perc=0.02,
)
...
CALLING_STATION = FuzzyPlayerArchetype(
    ...
    # [V24_EXTREME] V24 pushed this UP to 0.70 (not down, unlike the other 3 archetypes) because
    # CALLING_STATION's naturally low continue_bar meant even a small respect-application rate
    # swung too much of its equity range -- see that version's own comment for the full reasoning.
    # This diagnostic deliberately ignores that archetype-realism concern (maximizing the lever's
    # effect is the whole point of this run, not producing a deployable calling-station identity)
    # and pushes it to near-0 like the others, accepting CALLING_STATION may behave unrealistically
    # here. See versions/v24_extreme/SPECS.md.
    base_bluff_perc=0.02,
)
