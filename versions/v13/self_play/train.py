...
def simulate_worker(current_hand, bb_size, equity_sims, num_hands, hero_personality,
                    ...
                    ablate_hole_cards=False, range_aware_equity=False):
...
def run_training(personality, num_hands=100000, batch_size=256, epochs_per_batch=3,
    ...
    from versions.v13.self_play.telemetry import TrainingTelemetry
    ...
    if range_aware_equity:
        from versions.v13.self_play.simulator import _get_preflop_ranked
    ...
    try:
        while hands_done < num_hands:
            ...
            # Save past self checkpoint every 5,000 hands (skipped when the Past-Self seat
            # is disabled, so no stale mirror snapshot is written during a diagnostic run).
            if (not disable_past_self) and hands_done > 0 and (hands_done // 5000) > ((hands_done - len(batch_records)) // 5000):
