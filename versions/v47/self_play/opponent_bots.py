...
#
# Shapes follow each archetype's identity; weights are starting points VALIDATED by the C1
# calibration probe (versions/v47/self_play/calibrate_raise_sizes.py -- per-archetype size
# histograms + fold-vs-size response curves; adjust HERE if C1 flags a degenerate shape):
#   NIT             value-heavy, concentrated 0.66-1.0 pot, almost never jam-spam.
#   TAG             balanced spread around the old 0.75 center.
#   LAG             wide: min-raise probes (0.33), overbets (1.5x), occasional open-jam.
#   CALLING_STATION rarely raises at all (agg 0.15); when it does, small.
...
RAISE_SIZE_DISTRIBUTIONS = {
    'TAG':             [(0.50, 0.25), (0.75, 0.40), (1.00, 0.25), (1.50, 0.06), (None, 0.04)],
    'LAG':             [(0.33, 0.30), (0.66, 0.20), (1.00, 0.20), (1.50, 0.18), (None, 0.12)],
    'Nit':             [(0.66, 0.35), (0.75, 0.35), (1.00, 0.27), (None, 0.03)],
    'Calling Station': [(0.33, 0.50), (0.50, 0.35), (0.75, 0.15)],
}
...
def raise_size_distribution_for(style_or_name):
    """Resolve a pool style key ('maniac', 'fish', 'tag', ...) OR an archetype display name
    ('LAG', 'Calling Station', ...) to its raise-size distribution."""
    if style_or_name in RAISE_SIZE_DISTRIBUTIONS:
        return RAISE_SIZE_DISTRIBUTIONS[style_or_name]
    profile = BOT_PROFILES.get(str(style_or_name).lower())
    if profile is not None and profile.name in RAISE_SIZE_DISTRIBUTIONS:
        return RAISE_SIZE_DISTRIBUTIONS[profile.name]
    return _LEGACY_RAISE_DISTRIBUTION


def sample_raise_fraction(style_or_name):
    """Sample one raise event's pot fraction (None = all-in) from the archetype's repertoire."""
    dist = raise_size_distribution_for(style_or_name)
