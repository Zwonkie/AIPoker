...
from versions.v29.self_play.opponent_bots import TAG, LAG, NIT, CALLING_STATION, STYLE_SHIFT_SCALE
from versions.v29.self_play.opponents import HeuristicOpponent, build_opponent_pool
...
class SixMaxSimulator:
    ...
    def _query_model_decide(self, model, hand_cards, equity, pot_size, call_amount, hero_stack, num_opponents, table_state_dict=None, model_state_history=None, hero_actions_history=None, sample=True):
        ...
        from versions.v29.core.contract import ContractV12
        ...
        for idx in range(5):
            ...
            # [OPP-7] above) rather than the `stack=hero_stack` placeholder below (a pre-existing
            # approximation this change doesn't touch).
    ...
    def _mc_target_evs_sized(self, hero_cards, pot, to_call, hero_stack, street_idx,
        ...
        # [V30, BET-3] Multiway called-equity consistency fix. `base_pot_if_called` below uses a
        # SINGLE-caller pot geometry (pot + 2*raise_size - to_call), but the called-showdown equity
        # was charged at full-MULTIWAY `true_equity` -- an internal inconsistency that double-
        # penalizes a multiway jam (fold-equity credit collapses AND the called branch is charged
        # equity-vs-everyone) and taught the model to never raise with 3+ opponents (BET-3, live-
        # confirmed V29: won't jam even at 90% equity). A called short-stack jam is realistically
        # HEADS-UP vs one caller, so for >1 opponent we use the call-probability-weighted average of
        # hero's HU equity vs each opponent as the called-showdown equity -- consistent with the
        # one-caller geometry. HU (one opponent) is byte-for-byte unchanged (called_equity ==
        # true_equity). Postflop uses each opponent's real cards (same oracle basis as true_equity);
        # preflop this is oracle-HU (style comes from p_all_fold's fold-out, unchanged) -- a minor
        # approximation vs a caller's continuing range, far smaller than the multiway bias it fixes.
        multiway = len(active_opponents) > 1
        hero_hu_each = ([self._calculate_equity(hero_cards, board_str, 1, specific_opponents=[o['cards']])
                         for o in active_opponents] if multiway else None)
        ...
        for frac in raise_fracs:
            ...
            for i, (opp, oeq) in enumerate(zip(active_opponents, opp_base_eq)):
                ...
                fold_p = f / 10.0
                p_all_fold *= fold_p
                if multiway:                                    # accumulate this opp's call weight (for THIS size)
                    cw = 1.0 - fold_p
                    call_wsum += cw
                    called_eq_acc += cw * hero_hu_each[i]
            # [V30, BET-3] called-showdown equity consistent with the single-caller geometry
            if multiway and call_wsum > 1e-9:
                called_equity = called_eq_acc / call_wsum        # HU equity vs the likely caller(s)
            else:
                called_equity = true_equity                      # HU, or nobody ever calls -> unchanged
            base_pot_if_called = pot + 2.0 * raise_size - to_call
            ev_if_called = called_equity * base_pot_if_called - raise_size
            if not is_allin:
                # [V25] multi-street continuation correction -- see _rollout_continuation_ev.
                # Pass called_equity (not true_equity) so the rollout's own baseline subtraction
                # (trial_value - eq*base_pot) matches this branch's basis -- otherwise mismatched.
                ...
                ev_if_called += self._rollout_continuation_ev(
                    ...
                    called_equity, street_idx)
            raw_ev = p_all_fold * pot + (1.0 - p_all_fold) * ev_if_called
            if risk_aversion > 0.0:
                variance = self._outcome_variance(p_all_fold, pot, called_equity, base_pot_if_called, raise_size)
