...
from versions.v44.self_play.opponent_bots import TAG, LAG, NIT, CALLING_STATION, STYLE_SHIFT_SCALE
from versions.v44.self_play.opponents import HeuristicOpponent, build_opponent_pool
from versions.v44.core.contract import preflop_hand_strength, effective_contested_field
...
class SixMaxSimulator:
    ...
    def _query_model_decide(self, model, hand_cards, equity, pot_size, call_amount, hero_stack, num_opponents, table_state_dict=None, model_state_history=None, hero_actions_history=None, sample=True):
        ...
        from versions.v44.core.contract import ContractV12
    ...
    def _mc_target_evs_sized(self, hero_cards, pot, to_call, hero_stack, street_idx,
                             ...
                             last_raiser=-1, min_increment=None):
        ...
        for frac in raise_fracs:
            ...
            # [V43, review T-M9] "All-in" by CHIPS, not by label. `_raise_size_for_fraction` clamps
            # every fraction to `hero_stack`, so at short stacks raise_33/raise_66/raise_pot are
            # chip-identical to a shove -- yet only `frac is None` was treated as all-in, so the
            # other three collected `_ev_target_fold_decision`'s raise-only "show of strength" fold
            # bonus and the [V25] continuation credit for a stack hero does not have left. Four
            # "different" actions with different targets for ONE physical shove. Gated so the
            # calibration harness can measure the change before it is adopted (default off ==
            # V41 behaviour exactly).
                ...
                if not folded[current_actor] and stacks[current_actor] > 0:
                    ...
                    if current_actor == 0:  # Hero
                        ...
                        if decision == 'fold':
                            ...
                            # [V41, review #9] legal min-raise = to_call + last increment (was
                            # always +1bb), same rule the hero's own sizing now uses.
                            raise_size = max(raise_size, to_call + min_raise_inc)
