...
from versions.v17_gauntlet.self_play.opponent_bots import TAG, LAG, NIT, CALLING_STATION
...
def compute_range_aware_equity(hero_cards_str, board_str, opp_colors, noise=True, sims=150):
    ...
    consistency — see versions/v17_gauntlet/VALIDATED_FINDINGS.md). Returns equity in [0,1], or None if
    ...
    need = 5 - len(board0)
    # PREFLOP: opponents haven't acted, so each is only IN the pot with prob = their VPIP
    # (they fold the rest). POSTFLOP (board present): the still-active opponents have already
    # committed, so they're all in. This stops the preflop over-count (equity 0.17 vs 4 tight
    # seats that will mostly fold) — most fold, so hero is usually heads-up / wins uncontested.
        ...
        for color in opp_colors:
            ...
            for _t in range(25):
                ...
                if ai not in used and bi not in used:
                    used.add(ai); used.add(bi); hand = [ai, bi]; break
            if hand is None:
                ok = False; break
...
class SixMaxSimulator:
    ...
    def __init__(self, bb_size=10.0, equity_sims=200, hero_personality='main',
                 ...
                 tag_model=None, bootstrap_alpha=0.0):
        ...
        self.tag_model = tag_model  # [v17_gauntlet] NEW -- 'tag' was always heuristic-only before
    ...
    def _calculate_range_aware_equity(self, hero_cards_str, board_str, opp_colors, sims=None):
        ...
        eq = compute_range_aware_equity(
            ...
            noise=self.range_equity_noise, sims=sims or 150,
        )
        if eq is None:   # empty colors / bad hero / all rejected -> vs-random fallback
            return self._calculate_equity(hero_cards_str, board_str, len(opp_colors or []))
        return eq
    ...
    def _opponent_decide(self, seat_idx, opponent, equity, pot_odds, pot_size, stack, street_idx, table_state_dict=None, model_state_history=None, hero_actions_history=None):
        ...
        if is_preflop:
            ...
            # Action Forcing for Opponent personalities -- HEURISTIC BOTS ONLY (V17_gauntlet).
            # [v17_gauntlet] A real trained model already has its own learned tendencies; forcibly
            # overriding its decision toward a scripted archetype's target stats would mostly
            # replace the network's judgment with the forcing rule, defeating the point of loading
            # a genuinely skilled opponent. Scripted heuristic seats (model is None) are unaffected.
            if roll >= self.bootstrap_alpha and model is None:
            ...
            # Action Forcing for Opponent personalities -- HEURISTIC BOTS ONLY (V17_gauntlet).
            # Same rationale as the preflop branch above.
            if model is None:
                style = opponent.get('style')
                slot = STYLE_SLOT.get(style, seat_idx)
                opp_vpip = self.seat_histories[slot]['vpip_acts'] / max(1, self.seat_histories[slot]['vpip_ops'])
    ...
    def simulate_hand(self, current_hand=0):
        ...
        for s in range(1, 6):
            ...
            else:  # 'tag' -> TAG reference bot, OR a loaded model (falls back to TAG heuristic)
                opp_bot = self.tag_heuristic
                opp_model = self.tag_model
                opp_model = None
                ...
                if not folded[current_actor] and stacks[current_actor] > 0:
                    ...
                    if self.range_aware_equity and current_actor == 0:
                        ...
                        opp_colors = [
                            ...
                            for s in range(1, 6) if not folded[s]
                        ]
                        eq = self._calculate_range_aware_equity(hands_str[0], board_str, opp_colors)
                    else:
