...
from versions.v17_gauntlet.core.model import PokerEVModelV4
from versions.v17_gauntlet.self_play.simulator import SixMaxSimulator
from versions.v17_gauntlet.core.manifest import MANIFEST
...
# (see run_training's `elif hands_done < 30000` block) -- reused deliberately, not a new tuned
...
ACTOR_CRITIC_CUTOVER_HANDS = 30000
...
def regret_match_policy_torch(action_values, equity=None, baseline_mode='mean'):
    ...
    Also reproduces the SAME POLICY_TIGHTNESS_BB realization discount `vectorize_hand_samples`
    applies to the dataset-time actor target (via the `equity` context feature), so baseline_mode
    is the only thing this changes relative to round 1, not a second simultaneous change.
    ...
    if equity is not None and POLICY_TIGHTNESS_BB > 0.0:
        below_pivot = (POLICY_TIGHTNESS_PIVOT - equity).clamp(min=0.0) / POLICY_TIGHTNESS_PIVOT
        pen = POLICY_TIGHTNESS_BB * below_pivot          # [B, T]
...
def simulate_worker(current_hand, bb_size, equity_sims, num_hands, hero_personality,
    ...
    sim.tag_model = _load_worker_model(tag_model_path, required=False)  # [v17_gauntlet] NEW
...
def run_intermediate_sensitivity_check(model, step_label, device):
    ...
    from versions.v17_gauntlet.core.contract import ContractV12
...
def run_training(personality, num_hands=100000, batch_size=256, epochs_per_batch=3,
                 ...
                 nit_model_filename=None, tag_model_filename=None):
    ...
    #  * disable_target_shaping = the preflop-entry BIAS PRIOR (TIGHTNESS_PENALTY_BB) only.
    #    [V17] COUNTERFACTUAL_WEIGHT is DELIBERATELY NO LONGER zeroed by this flag -- it was
    #    bundled in here since V14, which left the critic's loss with real gradient for only
    #    FOLD + the taken action (weight 0 on every untaken action's counterfactual target).
    #    V17's actor-critic routing (below) needs the critic well-calibrated on ALL K actions,
    #    not just two of them, so COUNTERFACTUAL_WEIGHT now always stays at its module default
    #    (0.5) regardless of disable_target_shaping. See versions/v17_gauntlet/SPECS.md.
    global TARGET_CLIP_BB, TIGHTNESS_PENALTY_BB
    ...
    from versions.v17_gauntlet.self_play.telemetry import TrainingTelemetry
    ...
    if range_aware_equity:
        from versions.v17_gauntlet.self_play.simulator import _get_preflop_ranked
    ...
    try:
        while hands_done < num_hands:
            ...
            # Opponent model paths: maniac/fish (sticky) NNs disabled (heuristics only).
            # "Past Self" loads either a STATIC frozen expert (freeze_past_self) or the lagged
            # snapshot of THIS run once the first 5,000-hand checkpoint exists.
            # [v17_gauntlet] nit/tag repurposed to load STATIC frozen experts (see config.yaml
            # nit_model_filename/tag_model_filename) -- same pattern as frozen_past_filename.
            m_path = None
            n_path = (os.path.join(weights_dir, nit_model_filename)
                      if nit_model_filename and os.path.exists(os.path.join(weights_dir, nit_model_filename))
                      else None)
            s_path = None
            p_path = None if disable_past_self else (past_path if os.path.exists(past_path) else None)
            t_path = (os.path.join(weights_dir, tag_model_filename)
                      if tag_model_filename and os.path.exists(os.path.join(weights_dir, tag_model_filename))
                      else None)
...
if __name__ == '__main__':
    ...
    if isinstance(o_cfg, dict):
        ...
        nit_model_filename = o_cfg.get('nit_model_filename')  # [v17_gauntlet] frozen expert in 'nit' seat
        tag_model_filename = o_cfg.get('tag_model_filename')  # [v17_gauntlet] frozen expert in 'tag' seat
    else:
        ...
        nit_model_filename = tag_model_filename = None
    ...
    if opp_pool:
        ...
        if nit_model_filename:
            print(f"  Nit seat:        FROZEN {nit_model_filename} (static expert, unforced)")
        if tag_model_filename:
            print(f"  Tag seat:        FROZEN {tag_model_filename} (static expert, unforced)")
    ...
    print(f"  Target shaping:  {'preflop tightness prior OFF (counterfactual weight always ON, V17)' if disable_target_shaping else 'enabled'}")
    ...
    run_training(
        ...
        nit_model_filename=nit_model_filename, tag_model_filename=tag_model_filename,
    )
