...
# ======================================================================= #
#  V20 rescale constants (money-denominated context features).
# ======================================================================= #
# Inherited unchanged from V20 (see versions/v20/SPECS.md). Named here (not just inlined) so
# `train.py::vectorize_hand_samples` -- a SEPARATE context-vector builder used for the actual
# gradient-training tensors, which does NOT go through this class -- can import these same
# constants instead of re-deriving its own copy. That duplication is exactly what let V20's own
# rescale silently miss vectorize_hand_samples (it kept the old /400,/1000 scale while this file
# got the new /100,/250 scale) -- see versions/v20_preflopEq/SPECS.md and MANIFEST's docstring.
STACK_CEIL_BB = 50.0
POT_CEIL_BB = 100.0
CALL_CEIL_BB = 50.0
STACK_SCALE = 100.0
...
def equity_edge_feature(equity: float, num_active: int) -> float:
    """[V20_preflopEq] Equity's edge over the field-size fair share: 1.0 = exactly average for
    this field size, >1 better, <1 worse. `equity * (num_active + 1)`, deliberately NOT the plain
    `1/(num_active+1)` fair-share alone -- that's a deterministic unary function of the
    already-present `num_active` feature and would add ~nothing on its own; the genuinely hard
    part for a plain feedforward layer is the equity*N interaction, which this precomputes
    directly (same rationale as explicit cross-product features in factorization machines /
    wide-and-deep). See versions/v20_preflopEq/SPECS.md."""
...
class ContractV12(DataContract):
    """
    Implements the 37-feature context extraction for Pluribus V12 models (V20_preflopEq).

    35 features inherited from V20 unchanged, plus 2 new appended (not inserted -- every existing
    index is stable): `equity_edge` (ctx[35]) and `hand_strength` (ctx[36]). See
    versions/v20_preflopEq/SPECS.md.
    ...
    def to_tensors(self, states, hero_actions: list = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # 3. Context (37 features) Sequence
        context_seq = [[0.0]*37 for _ in range(self.max_seq_len)]
        ...
        for i, state in enumerate(states):
            ...
            ctx.append(equity_edge_feature(state.equity, total_active))
