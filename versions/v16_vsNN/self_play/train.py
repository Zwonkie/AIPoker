...
from versions.v16_vsNN.core.model import PokerEVModelV4
from versions.v16_vsNN.self_play.simulator import SixMaxSimulator
from versions.v16_vsNN.core.manifest import MANIFEST
...
def run_intermediate_sensitivity_check(model, step_label, device):
    ...
    from versions.v16_vsNN.core.contract import ContractV12
...
def run_training(personality, num_hands=100000, batch_size=256, epochs_per_batch=3,
    ...
    # nit_model_filename repurposes the otherwise-dormant 'nit' NN-opponent slot (fully wired
    # through simulate_worker already -- see nit_model_path there -- just never called with a
    # real path in any prior version) to load an ARBITRARY static frozen checkpoint, e.g. a
    # second independent NN opponent alongside the past-self mirror. v16_vsNN: frozen V15.
    nit_path = os.path.join(weights_dir, nit_model_filename) if nit_model_filename else os.path.join(weights_dir, 'expert_nit.pth')
    if nit_model_filename and not os.path.exists(nit_path):
        print(f"WARNING: nit_model_filename set but {nit_path} missing — Nit seat falls back to NIT heuristic.")
    ...
    from versions.v16_vsNN.self_play.telemetry import TrainingTelemetry
    ...
    if range_aware_equity:
        from versions.v16_vsNN.self_play.simulator import _get_preflop_ranked
    ...
    try:
        while hands_done < num_hands:
            ...
            # Opponent model paths: maniac/sticky NNs disabled (they use fuzzy heuristic bots).
            ...
            # "Nit" is repurposed (v16_vsNN) to a second, independent STATIC frozen opponent when
            # nit_model_filename is set -- otherwise stays disabled like maniac/sticky.
            m_path = None
            n_path = nit_path if (nit_model_filename and os.path.exists(nit_path)) else None
            s_path = None
...
if __name__ == '__main__':
    ...
    if isinstance(o_cfg, dict):
        ...
        nit_model_filename = o_cfg.get('nit_model_filename') or None  # repurpose Nit seat -> static frozen NN
    else:
    ...
    if opp_pool:
        ...
        if nit_model_filename:
            print(f"  Nit seat:        FROZEN {nit_model_filename} (static expert, repurposed slot)")
    ...
    run_training(
        ...
        nit_model_filename=nit_model_filename,
    )
