import os
import random
# [v46_legacySweep 2026-07-22] Pre-v30 versions are LEGACY by decision: V29 is deregistered, the
# 13-flag `is_vN` ladder, per-version bridge chain, `_LEGACY_LIVE_FEATURES` name-ladder, v9 river
# guardrail, math/bluff/preflop-chart override layers and the 3-way actor path are all REMOVED.
# Every registered engine DECLARES its behaviour (make_bridge/live_features/is_sized/display_tag/
# has_aux); an engine that doesn't is refused loudly. Legacy engine files, version slices and
# weights remain on disk (deprecate-not-delete) -- only shared-layer dispatch support ended.
from core.models.v14_engine import V14_ACTION_KEYS   # the shared 6-action key order
...
# ======================================================================= #
#  Live FEATURE providers -- the VERSION declares, the shared layer asks.
# ======================================================================= #
# History: PHPHelp.py once carried per-version substring ladders for equity/hand_strength that
# stopped at 'v29', so V40/V41 went live on vs-random equity and a constant hand_strength=0.5
# (V42_liveFixes' founding bug). The replacement `_LEGACY_LIVE_FEATURES` name-ladder that briefly
# lived here was itself removed in v46_legacySweep (pre-v30 = legacy): an engine's own
# `live_features()` declaration is now the ONLY mechanism, and a model without one is reported
...
class PokerDecisionEngine:
    def __init__(self, game_type="limit"):
        ...
        self.models = {
            ...
            # 2026-07-22 already. Every version's slice, weights and engine file remain on disk
            # (deprecate-not-delete, guardrails Rule 6 -- v13 and v41 are tagged MILESTONEs), but
            # re-registering a legacy model now requires giving its engine the declarations
            # (make_bridge/live_features/is_sized/display_tag/has_aux) -- the shared layer no
            # longer carries per-version dispatch for anything.
        }
        ...
        # substring ladder are gone -- a model without a bridge here is refused loudly at decision
        # time (never silently misrouted, never a wrong-contract fallback).
        ...
        for _name, _engine in self.models.items():
            ...
            if callable(_factory):
                try:
                    ...
                    print(f"ERROR: {_name} declares make_bridge() but it raised: {_e!r}. "
                          f"This model has NO tensor bridge and will refuse to act.")
    ...
    def _report_engine_health(self):
        ...
        for name, engine in self.models.items():
            ...
            route = 'own bridge' if name in self._engine_bridges else 'NO BRIDGE -- will refuse to act'
    ...
    def _resolve_live_spec(self, name: str):
        ...
        # [v46_legacySweep] The `_LEGACY_LIVE_FEATURES` name-ladder that used to back engines
        # without a declaration is gone: live_features() on the engine is the only mechanism.
        return None, 'unresolved', (
            f"'{name}'s engine declares no live_features() -- live equity would silently fall "
            f"back to vs-random. Add live_features() to its engine (see v44_engine.py).")
    ...
    def context_scales(self, model_name: str = None):
        ...
        # Historical pre-V20 scale -- only reachable when the version's contract can't be imported
        # or exports no scale constants. Always paired with a non-None `error`; decoded BB values
        # on this path are a GUESS. ([v46_legacySweep] the per-spec `context_scales` fallback
        # column died with the `_LEGACY_LIVE_FEATURES` name-ladder -- the contract's own module
        # constants are the single source.)
        ...
        stack, pot, call = fallback
        return {'stack': stack, 'pot': pot, 'call': call, 'source': f'{source}:fallback',
                'error': (f"{package}.core.contract exports no STACK_SCALE/POT_SCALE/CALL_SCALE -- "
                          f"decoded BB values for this model are a GUESS. Export the constants "
                          f"from that contract.")}
    ...
    def decide(self, obs, **kwargs):
        """[V45_liveHandover] The observation entry point: LiveObservation in, LiveDecision out.

        This is the ONLY call the live dashboard needs -- everything version-specific (equity
        estimator, hand_strength, front/after split, effective_field, tensor bridge, policy
        transforms) is resolved from what the ACTIVE engine itself declares, inside the adapter
        layer (core/live_adapter.py). A new version ships by declaring `live_features()` /
        `make_bridge()` / `is_sized` on its engine; no dashboard edit, no ladder edit.

        An engine may declare `make_live_adapter(decision_engine)` to supply a custom adapter
        (subclass of BaseLiveAdapter); otherwise the base pipeline is used. Adapters are cached
        per model name. kwargs pass through to BaseLiveAdapter.decide (evaluator, fallback_sims,
        fallback_num_opponents, log_fn).
        """
        from core.live_adapter import BaseLiveAdapter

        name = self.active_model_name
        if not hasattr(self, '_live_adapters'):
            self._live_adapters = {}
        adapter = self._live_adapters.get(name)
        if adapter is None:
            engine = self.models.get(name)
            factory = getattr(engine, 'make_live_adapter', None)
            if callable(factory):
                adapter = factory(self)
            else:
                adapter = BaseLiveAdapter(self, name)
            self._live_adapters[name] = adapter
        return adapter.decide(obs, **kwargs)
    ...
    def make_decision(self, board_state: BoardState,
        ...
        # [v46_legacySweep] The use_preflop_chart/use_math_engine/use_bluff_engine/
        # use_dynamic_sizing layer kwargs are gone: every override layer they gated was dead code
        # for sized models, and only sized models are registered.
        ...
        # [v46_legacySweep] Sized actor-policy engines are the only supported kind -- the legacy
        # 3-way and critic-argmax paths were removed with the pre-v30 versions. Refuse loudly
        # BEFORE touching the history buffers, so they stay aligned.
        if not getattr(active_model, 'is_sized', False):
            return 'FOLD', (f"Model '{self.active_model_name}' does not declare is_sized=True -- "
                            f"legacy engine paths were removed (v46_legacySweep); re-register it "
                            f"with a declaring engine"), 0.0, {}
        ...
        try:
            # [Fable review #16/H4, completed by v46_legacySweep] Engine-declared bridge only --
            # the 13-flag `is_vN` ladder and per-version bridge chain that lived here are gone.
            own_bridge = self._engine_bridges.get(self.active_model_name)
            if own_bridge is None:
                raise RuntimeError(
                    f"no tensor bridge for active model '{self.active_model_name}' -- its engine "
                    f"must declare make_bridge() (the version ladder was removed in "
                    f"v46_legacySweep; see v44_engine.py for the pattern).")
        ...
        # 1. Base Model Decision -- the sized 6-way actor policy {FOLD,CALL,RAISE_33,RAISE_66,
        # RAISE_POT,ALLIN}, the only supported path since v46_legacySweep. SAMPLE from it (matching
        # training/eval, which sample rather than argmax) with temperature sharpening, then
        # TRANSLATE the chosen raise bucket into a slider-sized live action.
        # Train/serve consistency: FOLD is strictly dominated when checking is free
        # (call_amount==0), so the sim zeroes it before selecting (simulator._select_action) -- we
        # mirror that here.
        ...
        if not bet_raise_available:
            for rk in V14_RAISE_FRAC:   # raise button gone -> only fold/call/check are legal
                probs[rk] = 0.0
        # [V42_liveFixes / Fable review M4] With no Check/Call button on screen (the all-in case
        # PHPHelp detects by button brightness), the sampler must never return CALL -- the executor
        # would click where no button exists. Mask it, exactly as the raise buckets are masked when
        # the raise button is gone.
        if not check_call_available:
            probs['CALL'] = 0.0
        # [V47, Change 2 / M9 -- train≡serve mirror] For an engine declaring
        # collapse_aliased_allin=True, a raise bucket whose slider chips equal the all-in amount
        # is the SAME physical action as ALLIN. V47 training collapses those buckets' actor-target
        # mass onto the canonical ALLIN bucket (train.py regret_match_policy* aliased masks), so
        # serving masks the chip-identical duplicates the same way before sampling -- the new
        # invariant pair. No engine before V47 declares this flag, so every deployed model's
        # distribution is untouched.
        if getattr(active_model, 'collapse_aliased_allin', False) and bet_raise_available:
            _allin_chips = float(getattr(board_state, 'hero_stack', 0.0) or 0.0)
            for rk, rfrac in V14_RAISE_FRAC.items():
                if rfrac is None:
                    continue   # the canonical ALLIN bucket itself
        ...
        # The temperature-sharpened, RENORMALIZED distribution the sampler actually draws from.
        ...
        if choice in V14_RAISE_FRAC:
            ...
            size_note = choice
        # [V43 / v46_legacySweep] `display_tag` is engine-declared; the 13-deep nested ternary that
        # backed undeclared engines died with the ladder. Fall back to the registry name itself,
        # which can never mislabel.
        _tag = getattr(active_model, 'display_tag', None) or self.active_model_name
        ...
        # Construct decision path for UI. [v46_legacySweep] The preflop-chart / bluff-engine /
        # math-engine override layers are gone (dead for every sized model); the model's own
        # policy is the only decision-maker.
        decision_path = {
            'active_model': {'status': "Active", 'details': f"Loaded {self.active_model_name}"},
            'legacy_layers': {'status': "Removed", 'details': "preflop_chart/bluff/math overrides removed in v46_legacySweep"},
        }
        ...
        _aux_ok = bool(getattr(active_model, 'has_aux', False))   # engine-declared only (v46)
