...
class PyTorchMLPModel(PokerModelInterface):
    def __init__(self, binary_path=None):
        if binary_path is None:
            binary_path = os.path.join(os.path.dirname(__file__), "binaries", "poker_mlp.onnx")
        self.binary_path = binary_path
        self.session = None
        self.action_map = {0: 'FOLD', 1: 'CHECK', 2: 'CALL', 3: 'BET', 4: 'RAISE'}
        
        # Load ONNX session if exists and onnxruntime is installed
        if os.path.exists(self.binary_path):
            try:
                import onnxruntime as ort
                self.session = ort.InferenceSession(self.binary_path)
            except Exception as e:
                print(f"[PyTorchMLPModel] Error loading ONNX runtime or model: {e}")
    ...
    def predict_action(self, board: list, hand: list, equity: float, pot_size: float, 
        ...
        if self.session is None:
            return 'FOLD', "PyTorch/ONNX Model not loaded (binaries/poker_mlp.onnx missing).", 0.0
        ...
        features = np.array([[
            ...
            float(num_opponents),
            equity,
            call_amount / (pot_size + call_amount) if (pot_size + call_amount) > 0 else 0.0,
            stack_pot_ratio,
            br_avail,
            cc_avail
        ]], dtype=np.float32)
        ...
        try:
            ...
            reason = f"PyTorch MLP ONNX Sampled (Class {pred_idx}). Dist: [{dist_str}]"
        except Exception as e:
            return 'FOLD', f"PyTorch MLP ONNX inference error: {e}", 0.0
        ...
        if not bet_raise_available and (action.startswith('BET') or action.startswith('RAISE')):
            if call_amount == 0:
                action = 'CHECK'
                reason = f"PyTorch predicted BET/RAISE but Bet/Raise button is unavailable. Checking."
                bet_size = 0.0
            else:
                action = 'FOLD'
                reason = f"PyTorch predicted BET/RAISE but Bet/Raise button is unavailable. Folding."
                bet_size = 0.0
                 
        if not check_call_available and action == 'CALL':
            action = 'RAISE'
            reason = f"PyTorch predicted CALL but Call button is unavailable. Shoving ALL-IN. (Original: {reason})"
