...
def vectorize_hand_samples(record, max_seq_len=20):
    ...
    for dp in dps:
        ...
        mc_return = (record.final_hero_profit + dp['committed_before']) / bb
        mc_return_seq[-1] = mc_return
        loss_mask[-1] = 1.0
        
        samples.append((hole_ints, board_seq, context_seq, action_seq, action_taken_seq, mc_return_seq, loss_mask))
    ...
    try:
        while hands_done < num_hands:
            ...
            for rec in batch_records:
                ...
                for h, b, c, a, sa, mc, mask in samples:
                    X_hole.append(h)
                    X_board.append(b)
                    X_ctx.append(c)
                    X_act.append(a)
            ...
            # Split train/val
            dataset = TensorDataset(hole_t, board_t, ctx_t, act_t, sa_t, mc_t, mask_t)
            ...
            for epoch in range(epochs_per_batch):
                ...
                epoch_loss = 0.0
                total_items = 0
                for b_h, b_b, b_c, b_a, b_sa, b_mc, b_m in train_loader:
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        b_sa_expanded = b_sa.unsqueeze(-1)
                        b_mc_expanded = b_mc.unsqueeze(-1).to(target.dtype)
                        target.scatter_(dim=2, index=b_sa_expanded, src=b_mc_expanded)
                        
                        # Target EV strictly equals true MC returns
                        b_ev_mod = target
            ...
            val_items = 0
            with torch.no_grad():
                for b_h, b_b, b_c, b_a, b_sa, b_mc, b_m in val_loader:
                    b_h = b_h.to(device)
                    b_b = b_b.to(device)
                    b_c = b_c.to(device)
                    b_a = b_a.to(device)
