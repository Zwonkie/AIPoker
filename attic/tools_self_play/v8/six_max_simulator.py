...
class SixMaxSimulator:
    ...
    def __init__(self, bb_size=10.0, equity_sims=200, hero_personality='main',
        ...
        # V8 Dynamic Opponent Profiling (last 20 hands history)
        # Track VPIP (voluntary preflop entry) and AGG (postflop bet/raise ratio)
        self.seat_histories = {s: {'vpip': [], 'agg': [], 'profit': 0.0} for s in range(6)}
    ...
    def _query_model_decide(self, model, hand_cards, equity, pot_size, call_amount, hero_stack, num_opponents, table_state_dict=None):
        ...
        state = {
            ...
            "hero_stack": hero_stack,
            "opponents": {},
            "num_active_players": num_opponents + 1,
            "street": street_str,
            "dealer_idx": 0,
            "hero_position": 0,
        ...
        h_t, b_t, c_t, a_t = MLBridge.state_to_tensors_v4(state)
        padding_mask = torch.tensor([[True] * 20], dtype=torch.bool)
        ...
        with torch.no_grad():
            preds = model(
                h_t.to(device), b_t.to(device), c_t.to(device), a_t.to(device),
                key_padding_mask=padding_mask.to(device)
            )
    ...
    def _calculate_analytical_target_evs(self, equity, pot, to_call, hero_stack, num_opponents):
        """Corrected V8 Analytical preflop target EV formula."""
        ev_fold = 0.0
        ...
        # Estimate Fold probability of opponents (Corrected V8 math)
        sizing_ratio = raise_size / max(1.0, pot)
        fold_factor = sizing_ratio / (sizing_ratio + 1.0)
        
        # 1. Equity Independence: fold rate determined preflop by profiles, not Hero's hole card equity
        p_fold_single = 0.70 * fold_factor
        
        # 2. Opponent Pot Odds: decay fold rate if Hero min-raises or offers great odds
        raise_increment = raise_size - to_call
        new_pot = pot + raise_size + (raise_size - to_call)
        pot_odds = raise_increment / max(1.0, new_pot)
        
        p_fold_single = p_fold_single * (1.0 - math.exp(-4.0 * pot_odds))
        
        # 3. Active Opponents Decay
        p_fold = p_fold_single ** num_opponents
        p_fold = min(0.95, max(0.0, p_fold))
        
        # Showdown EV if called
        ev_raise_if_called = equity * (pot + 2.0 * raise_size - to_call) - raise_size
        ev_raise = p_fold * pot + (1.0 - p_fold) * ev_raise_if_called
    ...
    def _hero_decide(self, equity, pot_size, call_amount, hero_stack, num_opponents, 
        ...
        # 1. 10% Pure Random Exploration to prevent off-policy data gaps
        roll = random.random()
        if roll < 0.10:
        ...
        if roll < 0.10 + model_prob and self.hero_model is not None:
            try:
    ...
    def simulate_hand(self, current_hand=0):
        ...
        # Compute VPIP / AGG from the last 20 hands running history
        opponents = []
        opponents_profiles = {}
        for s in range(1, 6):
            ...
            while not betting_ended:
                if not folded[current_actor] and stacks[current_actor] > 0:
                    ...
                    if current_actor == 0:  # Hero
                        ...
                        target_evs = self._calculate_analytical_target_evs(
                        ...
                        if street_idx > 0:
                            ...
                            if len(self.seat_histories[0]['agg']) > 20:
                        ...
                        if street_idx > 0:
                            ...
                            if len(self.seat_histories[current_actor]['agg']) > 20:
        ...
        for p in range(6):
            if preflop_had_decision[p]:
                ...
                if len(self.seat_histories[p]['vpip']) > 20:
