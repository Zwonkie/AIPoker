...
from tools.self_play.v8.six_max_simulator import SixMaxSimulator
...
def simulate_worker(current_hand, bb_size, equity_sims, num_hands, hero_personality,
    ...
    return records, sim.seat_histories
...
def print_dashboard(hands_done, total_hands, elapsed, hands_per_sec, 
                    ...
                    bootstrap_alpha, training_samples, total_hands_simulated):
    ...
    for s in range(6):
        ...
        seat_lines.append(f"|  - Seat {s} {seat_labels[s]:<16}: {bb100:>+6.1f} BB/100  (VPIP: {vpip:>4.1f}%, AGG: {agg:>4.1f}%)  |")
...
def run_intermediate_sensitivity_check(model, step_label, device):
    ...
    for label, cards, equity in preflop_hands:
        state = {
            ...
            "hero_position": 0,
            "opponents": {
                "seat_1": {"stack": 1000.0, "is_active": True, "vpip_color": "Green", "agg_color": "Green"}
            },
...
def run_training(personality, num_hands=100000, batch_size=256, epochs_per_batch=3,
    ...
    try:
        while hands_done < num_hands:
            ...
            for res, worker_hist in results:
