...
*   ⭐ [V17 Specs — DEPLOYED LIVE](file:///c:/REPO/Antigravity/AIPoker/.agents/skills/OFK/references/V17/specs.md): **CURRENTLY LIVE** (`core/models/v17_engine.py`, `Herocules (v17 Actor-Critic)`). Round 1 (mean-baseline critic routing) rejected — diluted FOLD even against a well-calibrated critic, wrong-direction VPIP drift. Round 2 (fold-relative baseline on the DENOISED critic Q, not raw noisy mc_evs) fixed it: `model_verify --full` 10 PASS/1 WARN/1 FAIL (FAIL = the same pre-existing deep-stack OOD every version carries). Fixes air/draws overcontinuation (air_folds_mostly 0.62->1.00) with NO style-flip regression (loose_deep BB/100 +62.1->+90.3, beats V16 outright; foldregret had collapsed this to -11.6). Also: telemetry bug found+fixed (free checks were silently conflated with paid air-calls in the equity-matrix dashboard) and an equity_sims MC cost/noise analysis (recommend reverting 5000->2000 for V18 — real ~2.5x CPU cost, marginal noise benefit).
*   [V18 Roadmap (planning only, no code yet)](file:///c:/REPO/Antigravity/AIPoker/.agents/skills/OFK/references/V18/specs.md): Backlog carried forward from V17. **[P0] deep-stack OOD elevated to top priority** (4 versions running unaddressed: V15/V16/foldregret/V17). [P5]/[P6] input-contract gaps promoted over further target-formula tuning (diminishing returns after 3 rounds of regret-matching tweaks). [P7] opponent-pool NN personalities (low priority). `equity_sims` should default to 2000, not V17's 5000 (analyzed, not just carried forward).