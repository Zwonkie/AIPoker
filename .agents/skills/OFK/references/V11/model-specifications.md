...
- **Multi-Action Target Generation (All-Action Loss):** The V10 training loop calculates MSE loss only on the action that was actually taken (`b_sa`). This allows unchosen actions to drift infinitely, causing the network to degenerate into a single action. V11 must calculate target Expected Values for *all three actions* (`Fold, Call, Raise`) using the math evaluator, constraining the entire EV prediction vector simultaneously.
  > [!NOTE]
  > **Evolution 2026-07-13 (this is now essentially correct, after a detour).** The V11 loss originally *masked* to the taken action only (the intent above was never wired up). It was first "fixed" with a Fold-only baseline (Fold→0), but that left Call/Raise unconstrained and the 200k model learned to **raise everything** (see the "Action-EV Extrapolation" entry in `issues-and-fixes.md`). The final design returns to genuine **all-action** training: Fold→0 and the taken action→realized `mc_return` at full weight, plus the **untaken** Call/Raise heads trained toward the simulator's true-equity MC counterfactual (`_calculate_mc_target_evs`) at weight `COUNTERFACTUAL_WEIGHT` = 0.5. Canonical description: `simulation_architecture.md` §4.
...
- **The Fix**: Both systems were patched on 2026-07-13. `six_max_simulator.py` now maps live seat histories to specific color bands dynamically during model inference. `contract_v11.py` and `train_selfplay.py` now calculate the active global norms dynamically by iterating over the `active_opponents_mask` and averaging the specific VPIP/AGG inputs for the active seats. This guarantees the model receives variance in its HUD inputs, allowing the linear `state_proj` layer to assign proper weight gradients.
  > [!WARNING]
  > **Correction (later on 2026-07-13).** The simulator-side portion of this fix was itself broken: `_query_model_decide` read `self.seat_histories[idx+1]['vpip']` / `['agg']`, keys that do not exist in the history buckets, raising `KeyError` on every call. Since both callers swallow the exception (`try/except: pass`), the model was **never actually queried** during self-play — every decision silently fell back to the heuristic chart, so the HUD-variance fix described here had no effect. Now repaired by threading the per-hand `opponents_profiles` (correct per-personality VPIP/AGG floats) into `_query_model_decide`. See the "Model Never Queried During Self-Play" entry in `issues-and-fixes.md`.

### 9. Inference Blindness Remediation (Left-Padding)
A severe "Monster-Under-The-Bed" syndrome occurred (folding 80%+ equity hands constantly) due to a misalignment between how the model was trained and how it was queried during live inference.
- **The Bug**: The training data used right-padding (padding index 0 to N and placing the actual states at the end of the 20-step buffer, meaning positional embeddings were consistently at `pos_emb[19]`). However, during inference, the `BoardState` was passed as a single element sequence, mapping to `pos_emb[0]`. The model learned actions relative to the end of the buffer, making it completely blind during inference.
- **The Fix**: Both training (`train_selfplay.py`) and inference (`contract_v11.py`) pipelines were modified to use **Left-Padding**. Sequences are now built by determining a `start_idx = max_seq_len - len(states)`, and padding is applied to the left (indices `0` to `start_idx-1`). Furthermore, `six_max_simulator.py` was updated to maintain and pass a running `model_state_history` buffer for each seat throughout the hand, ensuring that inference now receives a proper historical sequence mapped to the exact same positional embeddings used in training.
